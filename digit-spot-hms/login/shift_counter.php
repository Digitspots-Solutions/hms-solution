		...
		if(isset($shift_count_check) && $shift_count_check >= 1) {
			...
			if(!isset($time_error) || $time_error == 0) {
				...
				if($iscounter == true) {
					...
					?>
						<script> window.addEventListener('load', function() { objHidden('submit-box'); window.location.href = "<?php echo $_get_portal; ?>public/admin/portal<?php echo PHP_EXT.'?aps='.$_SESSION['app_service']; ?>"; }, false); </script>
					<?php
				}
			} else {
				$post_result .= '<div class="block-element top-push-5 bottom-push-5">';
