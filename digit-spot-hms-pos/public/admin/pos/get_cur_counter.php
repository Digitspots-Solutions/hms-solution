...
$order_key = array("posid"=>$cur_pos_store_id,"isreversed"=>0,"payment"=>"Pending","datelogged"=>$server_get_date,"deletedata"=>0);
$list_orders = mysqli_data_fetch($tbL100,'order_number',$order_key,'array');
if(is_array($list_orders)) { $datalist = ""; foreach($list_orders as $key => $val) { $datalist .= '<option value="'.$val['order_number'].'">'; } }

?>

<div class="fx-display-box fx-float-left nc-width-75">
	
	<?php if((isset($waiterDocket) && $waiterDocket == 'Yes') && (isset($allowDocket) && $allowDocket == 200)): ?>
		
		<span class="float-right"><a href="docket/" class="top-pull-10 right-pull-50 bottom-pull-10 left-pull-50 black-white-state ft-sml-size ft-tahoma rounded-button" target="_blank">Waiter Docket</a></span>
						
	<?php endif; ?>

	<?php if((isset($waiterDocket) && $waiterDocket == 'Yes') && (isset($allowOverseeDockets) && $allowOverseeDockets == 200)): ?>
		
		<span class="float-right top-pull-3 bottom-pull-10 right-push-30"><a href="docket-stack/" class="blue-font ft-sml-size ft-tahoma" target="_blank">+ Oversee Dockets</a></span>
						
	<?php endif; ?>

	<?php if((isset($allowDocket) && $allowDocket == 200) || (isset($allowOverseeDockets) && $allowOverseeDockets == 200)): ?>

	<span class="block-element new-line-space">
	</span>

	<span class="block-element cs-height-20">
	</span>

	<?php endif; ?>


	<?php if(isset($allowUseDesktopPOS) && $allowUseDesktopPOS == 200): ?>
	...
	<form action="pos/process_order.php" method="post" onsubmit="document.getElementById('orderbutton').value='Processing..'; setTimeout(() => { document.getElementById('orderbutton').setAttribute('type','button'); },200)" autocomplete="off">
		<div id="order-form" class="noshow">
			<div class="ln-display-box float-left nc-width-10 top-pull-10 bottom-push-7">
			...
			<div class="ln-display-box float-right nc-width-90 bottom-push-7">
				<span id="pending-order" class="ln-display-box float-left nc-width-35 grey-1-theme pads7 sml-rounded-button left-push-30" title="(UNCOMPLETED) Only today's order numbers allowed">
					<small class="default-text-font-bold ft-xxsml-size">Add bill to existing order</small>
					<input list="raisedbill" name="tobill" id="tobill" placeholder="Type here to choose order no" class="nopads no-back-black" autocomplete="off" onchange="pfr(this)">
					<datalist id="raisedbill">
						<?php echo $datalist; ?>
					</datalist>

					<!--<select name="pending-orders" id="pending-orders">
				...
				<?php
					if(isset($checkIfisfoodset) && $checkIfisfoodset == 'Yes') {
						?>
							<span class="ln-display-box float-right nc-width-30 left-push-10">
			...
			<div id="pos-header" class="block-element sml-rounded-button noscroll">
				<table cellpadding="0" cellspacing="0">
					...
					<tr>
						...
						<td width="150px" align="center">
							<input title="Enter maximum of two names with no symbols" type="text" name="guestname" id="guestname" onkeyup="getnames(this)" placeholder="Name e.g Will Smith" required="required">
		...
		<div id="ctrlbx" class="noshow alignct">
			<h4 class="large nobold default-text-font-bold bottom-pull-7 alignlt">Instruction before posting bill</h4>
			<ul>
				<li class="alignlt ft-xsml-size">Double-check your order to be sure is tallied</li>
				<li class="alignlt ft-xsml-size">Do not use add-to-bill for a completed order instead generate new bill</li>
				<li class="alignlt ft-xsml-size">Re-confirm payment option from guest before final post</li>
				<li class="alignlt ft-xsml-size">Contact your supervisor for reversal of any transaction</li>
			</ul>
			<p>&nbsp;</p>
			<input type="submit" name="submitbutton" id="orderbutton" value="Create Order" class="submit pads10 dark-black-white-state rounded-button nc-width-30"> &nbsp;&nbsp; <a href="?logs=<?php echo $logs; ?>" class="steel-blue-font">Cancel</a>
		</div>
	</form>
</div>
<div class="fx-display-box fx-float-right nc-width-20 box-border-thick sml-rounded-button pads10 obj-light-shadow">
...
<script>
	...
	function changeGuest(obj,strings) {
		if(strings == 'toroom') {
			...
			getfoodAcess(obj.value,3);
		} else if(strings == 'group') {
			document.getElementById('guestname').placeholder = 'Guest Account?';
			document.getElementById('guestname').required = true;
			$("#guestname").prop("readonly", false);
			getfoodAcess(obj.value,4);
			json_getdata(fguest,'cspg_tbl',obj.value,'id','code');
			...
			document.getElementById('guestname').required = true;
			$("#guestname").prop("readonly", false);
			getfoodAcess(obj.value,5);
		}

		if(strings == 'complimentary' || strings == 'group') {

			var xhtml, infrm;
			infrm = document.createElement('iframe');
			infrm.name = "counterframe";
			infrm.marginWidth = 0;
			infrm.marginHeight = 0;
			infrm.frameBorder = 0;
			infrm.width = "300px";
			infrm.height = "200px";
			infrm.scrolling = "auto";

			xhtml = '';
			xhtml += '<div class="cs-width-300 pads10 box-border-thick-bottom">';
			xhtml += '<span class="float-right top-pull-3"><a href="javascript:void(0)" class="black-font" onclick="xclcounter()" title="Close Notification"><b class="mbri-close"></b></a></span><h3 class="large nobold default-text-font-bold">Eligibilty Notification</h3>';
			xhtml += '</div>';

			document.getElementById('counter-notification').innerHTML = xhtml;
			document.getElementById('counter-notification').appendChild(infrm);

			infrm.src = "guest_counter_eligibility.php?checkby="+strings+"&customer="+obj.value+"&room=0";

			chgclass('counter-notification','fx-position-flow top-layout cs-margin-top-200 cs-margin-left-100 white-theme box-border-thick motion');
		}
	}
	...
	function xchangeGuest(dpx) {
		...
		infrm.src = "guest_counter_eligibility.php?checkby=room&customer="+guest+"&room="+room;
	...
	function getfoodAcess(guest,charge) {
		
		const pos = "<?php echo $cur_pos_store_id; ?>";

		if(charge == 2) {
			
			sqldatastring.sql = "SELECT isbill_to_room,billing_services FROM guest_tbl WHERE id="+guest;
			sqldataQuery(wgtpop,sqldatastring);

			function wgtpop(response) {
				var i, vhtml, data, ajaxresult = JSON.parse(response);
				data = ajaxresult.datastring[0];

				var fopt = '<option value="" selected="selected">Choose</option>';

				if(data.isbill_to_room == 'Yes') {
					var services = data.billing_services;
					var serv = (services).split(',');
					
					if(services.indexOf(pos+'-1') > -1) { fopt += '<option value="1">Breakfast</option>'; }
					if(services.indexOf(pos+'-2') > -1) { fopt += '<option value="2">Lunch</option>'; }
					if(services.indexOf(pos+'-3') > -1) { fopt += '<option value="3">Dinner</option>'; }

				} else {
					fopt += '<option value="">Not charged</option>';
				}

				writeObjheader('foodtype',fopt);
			}

			//$('#foodtype').prop('selectedIndex', 0);

		} else if(charge == 3) {
			
			/*sqldatastring.sql = "SELECT * FROM booking_invoice_tbl WHERE bill_to="+guest+" AND reservation='Checking In'";
			sqldataQuery(wgtpop,sqldatastring);
			...
			fopt = '<option value="" selected="selected">Choose</option>';
			fopt += '<option value="1">Breakfast</option>';
			fopt += '<option value="2">Lunch</option>';
			fopt += '<option value="3">Dinner</option>';

			writeObjheader('foodtype',fopt);

		} else if(charge == 4) {
			
			sqldatastring.sql = "SELECT * FROM cspg_pos_package_tbl WHERE cspgid="+guest;
			sqldataQuery(wgtpop,sqldatastring);
			...
			function wgtpop(response) {
				...
				if(data.length > 0) {
					for(var p=0; p < data.length; p++) {
						if((pos+'-1') == data[p].posid) { fopt += '<option value="1">Breakfast</option>'; }
						if((pos+'-2') == data[p].posid) { fopt += '<option value="2">Lunch</option>'; }
						if((pos+'-3') == data[p].posid) { fopt += '<option value="3">Dinner</option>'; }
					}
				} else {
					fopt += '<option value="">Not charged</option>';
				}
