											...
											<?php
												...
												if(is_array($groupdata2)) {
													...
													foreach($groupdata2 as $pos_ky => $pos_vl) {
														...
														$sql_in1 = "SELECT SUM(amount) AS totalchargeroom, SUM(discount) AS tdisc FROM {$tbL99} WHERE main_category={$ctg_vl['main_category']} AND posid={$pos_vl['posid']} AND billtype IN(2) AND status IN('Completed') AND isreversed=0 AND deletedata=0".$shift.$date_aQuery;

														$dataset_in1 = wgetSQL($sql_in1);

														$sql_in2 = "SELECT SUM(discount) AS totaldiscount FROM {$tbL99} WHERE main_category={$ctg_vl['main_category']} AND posid={$pos_vl['posid']} AND billtype IN(1,2,4,5) AND isreversed=0 AND status IN('Completed') AND deletedata=0".$shift.$date_aQuery;
														...
														$sql_in5 = "SELECT SUM(amount) AS totalchargecompl, SUM(discount) AS tdisc FROM {$tbL99} WHERE main_category={$ctg_vl['main_category']} AND posid={$pos_vl['posid']} AND billtype IN(3) AND status IN('Completed') AND isreversed=0 AND deletedata=0".$shift.$date_aQuery;
														...
														if($ctg_vl['main_category'] == 1) {
															$sql_in7 = "SELECT SUM(cover) AS totalcovers FROM {$tbL100} WHERE posid={$pos_vl['posid']} AND billtype IN(1,2,4,5) AND status IN('Completed') AND isreversed=0 AND deletedata=0".$shift.$date_aQuery;
														...
														$sql_in8 = "SELECT SUM(amount) AS totalrevenueincltax, SUM(discount) AS tdisc FROM {$tbL99} WHERE main_category={$ctg_vl['main_category']} AND posid={$pos_vl['posid']} AND billtype IN(1,2,4,5) AND status IN('Completed') AND isreversed=0 AND deletedata=0".$shift.$date_aQuery;
														...
														/*$sql_in9 = "SELECT SUM(tax_amount) AS vat, SUM(consumption_amount) AS consumption, SUM(service_charge_amount) AS scharge FROM {$tbL100} t1 LEFT JOIN {$tbL99} t2 ON t1.order_number=t2.order_number WHERE t2.main_category={$ctg_vl['main_category']} AND t1.posid={$pos_vl['posid']} AND t1.billtype IN(1,2,4,5) AND t1.isreversed=0 AND t1.deletedata=0 AND t1.datelogged BETWEEN '{$custom_date_start}' AND '{$custom_date_end}'".$nshift;

														$dataset_in9 = wgetSQL($sql_in9);*/
														...
														/*$sql_in10 = "SELECT SUM(qty) AS totalqty, SUM(cost) AS totalcost FROM {$tbL99} WHERE main_category={$ctg_vl['main_category']} AND posid={$pos_vl['posid']} AND billtype IN(1,2,4,5) AND isreversed=0 AND deletedata=0".$shift.$date_aQuery;*/

														$sql_in10 = "SELECT qty,cost FROM {$tbL99} WHERE main_category={$ctg_vl['main_category']} AND posid={$pos_vl['posid']} AND billtype IN(1,2,4,5) AND status IN('Completed') AND isreversed=0 AND deletedata=0".$shift.$date_aQuery;
