...
include "../module_operation_privilege.php";
	...
	if(isset($_GET['cfm']) && $_GET['cfm'] == 'y') {
		...
		if($ca == 'cancel' || $ca == 'delete') {
			...
			<script>
				window.addEventListener('load',function() {
					window.location.href = '../pos_open_counter.php?logs=<?php echo $posname; ?>';
				},false);
			</script>
			<?php
		}
	...
	if(!empty($new_order)) {
		...
		$invoice_data_property = "order_number,invoice_number,posid,customerid,discount_amount,sub_total,tax_amount,bill_amount,foodtype,billtype,tableid,cover,detail,status,datelogged,timelogged,biller,roomid,isreversed,ispaid,consumption_amount,service_charge_amount";
		...
		?>
			...
			<div class="block-element top-pull-10">
															...
															<div class="block-element box-border-thick sml-rounded-button alignrt">
																<div class="block-element box-border-thick-bottom pads10">
																	...
																	<?php
																		...
																		if(!empty($get_invoice_data[20]) && $get_invoice_data[20] > 0) {
																			?>
																				<span class="ln-display-box float-left">
																					<h4 class="large">Consumption Tax</h4>
																				</span>
																				<span class="ln-display-box float-right">
																					<h4 class="large nobold"><?php echo number_format($get_invoice_data[20],2); ?></h4>
																				</span>
																				<span class="block-element new-line-space">	
																				</span>
																			<?php
																		...
																		if(!empty($get_invoice_data[21]) && $get_invoice_data[21] > 0) {
																			?>
																				<span class="ln-display-box float-left">
																					<h4 class="large">Service Charge</h4>
																				</span>
																				<span class="ln-display-box float-right">
																					<h4 class="large nobold"><?php echo number_format($get_invoice_data[21],2); ?></h4>
																				</span>
																				<span class="block-element new-line-space">	
																				</span>
																			<?php
				...
				<div class="ln-display-box float-right nc-width-25 box-border-thick sml-rounded-button pads20 obj-light-shadow right-push-10">
					...
					<form action="process_open_payment.php" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
						...
						<?php /*if($get_invoice_data[19] == 0) { ?><p class="top-pull-15 alignct"><a href="?q=<?php echo $new_order; ?>&a=cancel&rq=<?php echo $new_order; ?>&r=new_order&new_order=<?php echo $new_order; ?>" class="blue-font ft-sml-size">Cancel Order</a></p><?php }*/ ?>
						
						<?php if($get_invoice_data[19] == 0 && $get_invoice_data[18] == 0) { if((isset($reverseday) && $reverseday <= $gh_get_allow_past_reverse) && (isset($allowPosReverse) && $allowPosReverse == 200)) { ?><p class="top-pull-15 alignct"><a href="javascript:void(0)" class="blue-font top-pull-10 right-pull-30 bottom-pull-10 left-pull-30 box-border-thick rounded-button obj-light-shadow ft-sml-size" onclick="popmodalframe('','posreversemode','<?php echo $new_order; ?>','reverse',400,400);">Reverse Order</a></p><?php } } ?>
		...
		if(payid !== null && payid != '') {
			...
			function wgtpop(response) {
				...
				if(data.paytype == 'Two-way Payment') {
					...
					vhtml += '<ul class="nolist">';
					vhtml += '<li class="ln-display-box float-left nc-width-50 top-pull-7 right-pull-5">';
					vhtml += '<input type="number" min="1" name="wgtf1" id="wgtf1" placeholder="0.00" value="Acquiring..">';
					vhtml += '</li>';
					vhtml += '<li class="ln-display-box float-left nc-width-50 top-pull-7 left-pull-5">';
