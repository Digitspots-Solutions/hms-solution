...
<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		...
		&nbsp; Note: create pos tables by clicking <u>new table</u> button
 	</span>
...
<?php
	...
	{
		createDatabasetable($var_tbl_15); //create a table for this post
		...
		$insert_dataproperty = array("postoreid"=>$cur_pos_store_id,"tablename"=>ucwords(strtolower($fieldset1)),"tabletype"=>$fieldset3,"noofperson"=>$fieldset4,"tableshape"=>$fieldset5,"tablesize"=>$fieldset6,"detail"=>$fieldset2);
		$insert_constrain = "";
		$data_inserted = mysqli_data_insert($tbL17,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Create new pos table","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
				...
				<div class="nc-width-40">
					<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
						<div class="bottom-push-20 alignlt">
							<h3 class="nomargin">Creating New Pos Table</h3>
						</div>
						<span class="block-element bottom-push-10">
							<input type="text" name="fieldset1" id="fieldset1" placeholder="Enter table name" required="required">
						</span>
						<span class="block-element bottom-push-10">
						...
						<span class="block-element bottom-push-10">
							<select name="fieldset3" id="fieldset3" required="required">
								<option value="" selected="selected">Table Type</option>
								<option value="Single Table">Single Table</option>
								<option value="Multiple Table">Multiple Table</option>
							</select>
						</span>
						<span class="block-element bottom-push-10">
							<input type="text" name="fieldset4" id="fieldset4" pattern="\d*" placeholder="Enter number of person" required="required">
						</span>
						<span class="block-element bottom-push-10">
							<select name="fieldset5" id="fieldset5" required="required">
								<option value="" selected="selected">Table Shape</option>
						...
						<span class="block-element bottom-push-10">
							<input type="text" name="fieldset6" id="fieldset6" placeholder="Enter table size">
						</span>
	...
	if(isset($_POST['editbutton']))
	{
		...
		$insert_dataproperty = array("tablename"=>ucwords(strtolower($fieldset1)),"tabletype"=>$fieldset3,"noofperson"=>$fieldset4,"tableshape"=>$fieldset5,"tablesize"=>$fieldset6,"detail"=>$fieldset2);
		$insert_constrain = array("id"=>$fieldset7);
		$data_inserted = mysqli_data_update($tbL17,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Edit pos table details","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['deletebutton']) && isset($_POST['checkers']))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$del = mysqli_data_update($tbL17,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Remove from pos table list","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if((isset($_POST['searchbutton'])) && (isset($_POST['search']) && !empty($_POST['search']))) {
		$keywords=" AND (tablename LIKE '".escape_data($_POST['search'])."%' OR tabletype LIKE '".escape_data($_POST['search'])."%' OR tableshape LIKE '".escape_data($_POST['search'])."%')";
	} else { 
		$keywords="";
	}
	...
	$dataproperty = "id,tablename,tabletype,noofperson,tableshape,tablesize,detail,status";
	$constrain = array("deletedata"=>0,"postoreid"=>$cur_pos_store_id);
	$row = "array";

	$dataCollect = mysqli_data_fetch($tbL17,$dataproperty,$constrain,$row);

	if(is_array($dataCollect))
	{
		$thproperty = array("noth","name","type","shape","size","no of person","description","noth","enoth");
		...
		foreach($dataCollect as $theader => $tdata)
		{
			...
			$htmlresult .= '<td width="200px" align="center" class="box-border-thick-right">'.$tdata["tablename"].'</td>';
			$htmlresult .= '<td width="100px" align="center" class="box-border-thick-right">'.$tdata["tabletype"].'</td>';
			$htmlresult .= '<td width="100px" align="center" class="box-border-thick-right">'.$tdata["tableshape"].'</td>';
			$htmlresult .= '<td width="100px" align="center" class="box-border-thick-right">'.$tdata["tablesize"].'</td>';
			$htmlresult .= '<td width="100px" align="center" class="box-border-thick-right">'.$tdata["noofperson"].'</td>';
			...
			if((isset($_GET['edit']) && $_GET['edit'] >= 1) && ($_GET['edit'] == $dataid))
			{
				...
				$htmlresult .= '<h4 class="large blue-font">Updating Pos Table</h4><br>';
				$htmlresult .= '<div class="nc-width-40">';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Table Name</small>';
				$htmlresult .= '<input type="text" name="fieldset1" id="fieldset1" placeholder="Enter table name" value="'.$tdata["tablename"].'" required="required">';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				...
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Table Type</small>';
				$htmlresult .= '<select name="fieldset3" id="fieldset3" required="required">';
				$htmlresult .= '<option value="'.$tdata["tabletype"].'" selected="selected">'.$tdata["tabletype"].'</option>';
				$htmlresult .= '<option value="Single Table">Single Table</option>';
				$htmlresult .= '<option value="Multiple Table">Multiple Table</option>';
				$htmlresult .= '</select>';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Number of Person</small>';
				$htmlresult .= '<input type="text" name="fieldset4" id="fieldset4" pattern="\d*" placeholder="Enter number of person" value="'.$tdata["noofperson"].'" required="required">';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Table Shape</small>';
				$htmlresult .= '<select name="fieldset5" id="fieldset5" required="required">';
				$htmlresult .= '<option value="'.$tdata["tableshape"].'" selected="selected">'.$tdata["tableshape"].'</option>';
				$htmlresult .= '<option value="Square">Square</option>';
				$htmlresult .= '<option value="Rectangle">Rectangle</option>';
				$htmlresult .= '<option value="Circle">Circle</option>';
				$htmlresult .= '</select>';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Table Size</small>';
				$htmlresult .= '<input type="text" name="fieldset6" id="fieldset6" placeholder="Enter table size" value="'.$tdata["tablesize"].'">';
				$htmlresult .= '</span>';
				$htmlresult .= '</div>';
	...
	$additionalQuery = "";
	mysqli_data_check($tbL17,'(*)',$constrain);
