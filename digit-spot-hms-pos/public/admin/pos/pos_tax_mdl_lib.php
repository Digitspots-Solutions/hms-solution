...
<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		...
		&nbsp; Note: create pos tax by clicking <u>new tax</u> button
 	</span>
...
<?php
	...
	{
		createDatabasetable($var_tbl_16); //create a table for this post
		...
		$insert_dataproperty = array("postoreid"=>$cur_pos_store_id,"taxname"=>ucwords(strtolower($fieldset1)),"taxcharge"=>$fieldset2,"detail"=>$fieldset3);
		$insert_constrain = "";
		$data_inserted = mysqli_data_insert($tbL18,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Create new pos tax charge","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
					...
					<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
						...
						<span class="block-element bottom-push-10">
							<div class="ln-display-box float-left nc-width-90">
								<input type="number" name="fieldset2" id="fieldset2" step="any" placeholder="Enter tax charges" required="required">
							</div>
	...
	if(isset($_POST['editbutton']))
	{
		...
		$insert_dataproperty = array("taxname"=>ucwords(strtolower($fieldset1)),"taxcharge"=>$fieldset2,"detail"=>$fieldset3);
		$insert_constrain = array("id"=>$fieldset6);
		$data_inserted = mysqli_data_update($tbL18,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Edit pos tax details","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['deletebutton']) && isset($_POST['checkers']))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$del = mysqli_data_update($tbL18,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Remove pos tax list","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if((isset($_POST['searchbutton'])) && (isset($_POST['search']) && !empty($_POST['search']))) {
		$keywords=" AND taxname LIKE '".escape_data($_POST['search'])."%'";
	} else { 
		$keywords="";
	}
	...
	$dataproperty = "id,taxname,taxcharge,detail,status";
	$constrain = array("deletedata"=>0,"postoreid"=>$cur_pos_store_id);
	$row = "array";

	$dataCollect = mysqli_data_fetch($tbL18,$dataproperty,$constrain,$row);

	if(is_array($dataCollect))
	{
		$thproperty = array("noth","tax name","charges","description","status","noth","enoth");
		...
		foreach($dataCollect as $theader => $tdata)
		{
			...
			$htmlresult .= '<td width="200px" align="center" class="box-border-thick-right">'.$tdata["taxname"].'</td>';
			$htmlresult .= '<td width="100px" align="center" class="box-border-thick-right">'.$tdata["taxcharge"].' %</td>';
	...
	$additionalQuery = "";
	mysqli_data_check($tbL18,'(*)',$constrain);
