<?php
	...
	//get all available outlets for request
	$get_posstore = select_dt_fetch('status','Active',$tbL14,'id','posname');
?>
...
<div class="bottom-push-20">
	<h4 class="large nobold">Search by:</h4><br>
	<ul class="nolist">
		<li class="ln-display-box float-left nc-width-30 top-pull-2 right-pull-30"><select id="searchlist" class="nopads no-back-black"><option value="" selected>Available Outlets</option><?php echo $get_posstore; ?></select></li>
		<li class="ln-display-box float-left nc-width-20 top-pull-2 left-pull-10"><select id="searchstatus" class="nopads no-back-black" onchange="changeList()"><option value="" selected>Status</option><option value="">All</option><option value="Transfer Completed">Transfer Completed</option><option value="Under Approval">Under Approval</option></select></li>
...
<h3 class="large nobold black-font alignlt">Outlet to outlet pending transfer: <?php echo $noofpending; ?></h3><br>
...
<?php
	...
	if(isset($_GET['storeid']) && $_GET['storeid'] > 0) {
		$additionalQuery = " AND to_posid={$_GET['storeid']} GROUP BY transfer_number ORDER BY id DESC";
		$constrain = array("deletedata"=>0);
		if(isset($_GET['stat']) && !empty($_GET['stat'])) { $constrain['transfer_status'] = $_GET['stat']; }
	} else {
		$additionalQuery = " GROUP BY transfer_number ORDER BY id DESC";
		$constrain = array("deletedata"=>0,"transfer_status"=>"Under Approval");
	}


	$dataproperty = "transfer_number,datelogged";
	...
	if(is_array($dataGroup))
	{
		...
		foreach($dataGroup as $theader => $tdata)
		{
			...
			$constrain = array("transfer_number"=>$tdata['transfer_number'],"deletedata"=>0);
			if(isset($_GET['stat']) && !empty($_GET['stat'])) { $constrain['transfer_status'] = $_GET['stat']; }
			else { $constrain['transfer_status'] = "Under Approval"; }
			$dataCollect = mysqli_data_fetch($tbL157,$dataproperty,$constrain,'array');

			if(is_array($dataCollect))
			{
				$thproperty = array("enoth","from store","to outlet","item","qty to transfer","type","logged by","status");
				...
				$htmlresult .= '<h3 class="large nobold">Transfer No: <b class="nobold default-text-font-bold right-push-5">'.$tdata['transfer_number'].' ('.date('d-m-Y',strtotime($tdata['datelogged'])).')</b> <a href="javascript://" class="blue-font ft-xsml-size" title="'.$tapx.'">&hookrightarrow; See report status</a></h3>';
				...
				$disabled=""; $isdisbursed=0; $outlet_name=""; $outlet_name2=""; $store_name="";
				...
				foreach($dataCollect as $theader => $tdata)
				{
					...
					$store_name = idget_data($tbL123,$tdata['from_posid'],'store_name');
					
					if($tdata['to_posid'] > 0) { $outlet_name2 = idget_data($tbL14,$tdata['to_posid'],'posname'); } else { $outlet_name2 = "Warehouse"; }
					...
					$htmlresult .= '<td width="100px" align="center" class="box-border-thick-right">'.$store_name.'</td>';
...
<script>
	...
	function getPoStores(id) {
		...
		if(document.getElementById(id).value == '' || document.getElementById(id).value == null) {
			...
			sqldatastring.sql = "SELECT id,posname FROM pos_store_tbl WHERE iscounter IN('Yes','No') AND status IN('Active') AND deletedata=0";
			sqldataQuery(wgtpop,sqldatastring);
