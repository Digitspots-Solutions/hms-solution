									...
									<?php
										...
										if(is_array($wgtGdata) && count($wgtGdata)) {
											...
											foreach($wgtGdata as $key => $val) {
												...
												$isSql2 = "SELECT costprice,datelogged FROM {$tbl} WHERE itemid={$val['itemid']} AND deletedata=0 ORDER BY id DESC"; $forCP = idget_data($isSql2);
												...
												foreach($forCP as $key2 => $val2) {
													$incre += 1;
													if($incre == 1) { $costprice_option .= '<option value="'.$val2['costprice'].'">'.$val2['costprice'].' - '.date('d-m-Y',strtotime($val2['datelogged'])).' &check;</option>'; } else { $costprice_option .= '<option value="'.$val2['costprice'].'">'.$val2['costprice'].' - '.date('d-m-Y',strtotime($val2['datelogged'])).' x</option>'; }
												}
												...
												?>
													<tr class="white-grey-state motion">
														...
														<td class="cs-width-300 alignct right-pull-7"><select class="nopads no-back-black"><?php echo $costprice_option; ?></select></td>
													</tr>
												<?php
