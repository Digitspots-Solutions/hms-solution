<?php
	...
	if(isset($_GET['orderno'])) { $keywords = " AND order_number='{$_GET['orderno']}' AND gstat IN('Pending')".$toStore; $disableHeaders = 1; } else { $keywords = " AND gstat IN('Pending')".$toStore; $disableHeaders = 0; }
	...
	//get only pending approval orders number
	$lAr = "SELECT * FROM {$tbl} WHERE gstat IN('Pending') AND deletedata=0".$toStore." GROUP BY order_number";
...
<div class="pads30">
	...
	<div class="float-right cs-width-250 box-border-thick sml-rounded-button pads15 alignlt">
		<h4 class="xlarge nobold default-text-font-bold">New PRs</h4>
	...
	<div class="alignlt"><h3 class="xlarge nobold nomargin"><a href="<?php echo $ths_page; ?>" title="Refresh" class="right-push-10"><img src="<?php echo DOMAIN_URL; ?>theme/images/general/refresh.png"></a> here are list of purchase orders to be submitted for approval</h3></div>
									...
									<?php
										if($val['gstat'] == 'Pending') {
											?>
												...
												<a href="javascript:void(0)" class="blue-font box-border-thick top-pull-7 right-pull-20 bottom-pull-7 left-pull-20 rounded-button white-theme obj-light-shadow right-push-10" onclick="jsEdit(); callupPr('<?php echo $val['order_number']; ?>')" title="Editing PR">Edit <b class="fas fa-edit left-push-5"></b></a> <a href="javascript:void(0)" class="blue-white-state top-pull-7 right-pull-20 bottom-pull-7 left-pull-20 rounded-button right-push-10" onclick="wgtfsubmit('datasheet','delete')" title="Use checkbox to delete">Delete  <b class="fas fa-trash-alt left-push-5"></b></a>
											...
											if($val['order_status'] == 'Approved') {
												...
												?>
													Order No: <b class="royal-blue-font"><?php echo $val['order_number']; ?></b><br>
													Status: <b class="light-red-font">Under Approval</b><br><br>
									...
									<div class="cs-width-2000">
										<table cellspacing="0" cellpadding="0">
												...
												foreach($wgt_supplier as $key2 => $val2) {
													...
													?>
														<tr>
															...
															<td class="right-pull-10 left-pull-10"><?php echo number_format($val2['order_total_amount']); ?></td>
											...
											<tr>
												...
												<td class="yellow-theme nunito-bold right-pull-10 left-pull-10"><?php echo number_format($total_amount); ?></td>
												<td class="default-text-font-bold right-pull-10 left-pull-10">&nbsp;</td>
												<td class="default-text-font-bold right-pull-10 left-pull-10">&nbsp;</td>
												<td class="default-text-font-bold right-pull-10 left-pull-10">&nbsp;</td>
											</tr>
...
<script>
	...
	function callupPr(order) {
		...
		var endfetch = setInterval(() => {
			if(document.getElementById('table-form')) {
				...
				sqldatastring.sql = "SELECT t1.id,t1.order_number,t1.supplierid,t1.itemid,t1.unitprice,t1.qty_ordered,t1.order_total_amount,t2.supplier_name,t3.item FROM stock_item_purchase_order_tbl t1, supplier_tbl t2, stock_item_tbl t3 WHERE t1.order_number='"+order+"' AND t1.supplierid = t2.id AND t1.itemid = t3.id";
				...
				function wgtpop(response) {
					...
					for(i=0; i<data.length; i++) {
						...
						vhtml += '<tr>';
						vhtml += '<td class="nunito-semibold right-pull-10 left-pull-10 cs-width-200"><select name="supplier[]" id="supplier'+nrow+'" class="nopads no-back-black"><option value="'+data[i].supplierid+'">'+data[i].supplier_name+'</option></select><input type="hidden" name="rwid[]" value="'+data[i].id+'"></td>';
						...
						vhtml += '<td class="nunito-semibold right-pull-10 left-pull-10"><input list="thecostprice'+nrow+'" name="unitcost[]" id="unitcost'+nrow+'" placeholder="Type to choose?" class="nopads no-back-black unitcost" oninput="nextTp('+nrow+')" value="'+data[i].unitprice+'" readonly required><datalist id="thecostprice'+nrow+'"></datalist></td>';
						vhtml += '<td class="nunito-semibold right-pull-10 left-pull-10"><input type="number" min="1" name="quantity[]" id="quantity'+nrow+'" placeholder="Enter quantity?" value="'+data[i].qty_ordered+'" class="nopads no-back-black quantity" onkeypress="nextRow('+nrow+',this.id,event)" onblur="lastRow('+nrow+',this.id)" required></td>';
	...
	function getrqi(obj) {
		window.location.href = window.location.href+"&orderno="+obj.value;
	}
