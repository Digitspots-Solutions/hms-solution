<div class="alignlt left-pull-30 bottom-push-30"><h3 class="large nobold nomargin">Here are the list of awaiting items to be transferred. Please acknowledge your consent</h3></div>
<div class="pads30">
	...
	<?php
		...
		if(!empty($get_requestno) && $get_requestno != '') { $keywords = "transfer_number='{$get_requestno}' AND deletedata=0 AND transfer_status IN('Under Approval') GROUP BY transfer_number"; } else { $keywords = "deletedata=0 AND transfer_status IN('Under Approval') GROUP BY transfer_number"; }
		...
		if(is_array($wgt_ir)) {
			...
			foreach($wgt_ir as $key => $val) {
				...
				$sqldata2 = "SELECT * FROM {$tbL156} WHERE transfer_number='{$val['transfer_number']}' AND deletedata=0 AND transfer_status IN('Under Approval')"; $wgt_ir2 = idget_data($sqldata2);
				...
				?>
					<form action="" method="post" autocomplete="off">
						...
						<h3 class="large nobold nunito-bold alignlt">+ Transfer Item #<?php echo $val['transfer_number']; ?></h3>
						<!--<input type="hidden" name="uri" value="edit-item-transfer">-->
						<input type="hidden" name="requestno" value="<?php echo $val['transfer_number']; ?>">
							...
							<div class="cs-width-1200">
								<table cellspacing="0" cellpadding="0">
									<tr>
										...
										<td class="default-text-font-bold right-pull-10 left-pull-10">Transfer Type</td>
										<td class="default-text-font-bold right-pull-10 left-pull-10">Transfer By</td>
									</tr>
									<?php
										$numb = 0;
										...
										foreach($wgt_ir2 as $key2 => $val2) {
											...
											?>
												<tr>
													...
													<td class="right-pull-10 left-pull-10"><?php echo ucfirst($val2['tagged_name']); ?></td>
...
<script>
	...
	function signPr(nos,id,level) {
		...
		if(wgets.value != '' && wgets.value !== null) {
			...
			vhtml += '<form action="" method="post" autocomplete="off" onsubmit="istr(this)" data-field="'+wgets+'">';
			vhtml += '<input type="hidden" name="uri" value="apply-item-transfer-approval">';
	...
	function istr(obj) {
		...
		setTimeout(() => {
			document.getElementById('approvebutton').type = 'button';
			document.getElementById('approvebutton').value = 'Applying, wait..';
		},1000);
	}
