<?php
	...
	$keywords = " AND receipt_status IN('Pending') AND var_status NOT IN(3) AND pr_status IN('IOU','IOU Approved') AND var_approval='Yes'";
...
<div class="pads30">
	<form action="" method="post" autocomplete="off" id="for-stock">
		<input type="hidden" name="uri" value="apply-receive-stock">
		<input type="hidden" name="orderno" id="orderno">
		
		<div class="alignlt"><h3 class="large nobold nomargin"><a href="<?php echo $ths_page; ?>" title="Refresh" class="right-push-10"><img src="<?php echo DOMAIN_URL; ?>theme/images/general/refresh.png"></a> Here are list of approved stock to be received. Confirm and receive to add-up to stock</h3></div>
		...
		<?php
			...
			if($po['isdata'] == true) {
				...
				$isSql = "SELECT * FROM {$tbl} WHERE deletedata=0".$keywords." AND order_net_amount > 0 AND var_approval='Yes' GROUP BY order_number ORDER BY id DESC";
				$wgt_po = idget_data($isSql);

				if(is_array($wgt_po) && count($wgt_po)) {
					foreach($wgt_po as $key => $val) {
						
						$query_stock = "SELECT * FROM {$tbl} WHERE order_number='{$val['order_number']}' AND order_net_amount > 0 AND var_approval='Yes' AND deletedata=0";
