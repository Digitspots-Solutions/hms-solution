<?php
	...
	if(isset($_GET['orderno']) && !empty($_GET['orderno'])) { $iou_no = remove_data_injection($_GET['orderno']); $keywords = " AND iou_no='{$_GET['orderno']}' AND status IN('Under Approval','Pending','Approved')"; $disableHeaders = 1; } else { $keywords = " AND status IN('Under Approval')"; $disableHeaders = 0; $iou_no = ""; }
	...
	<form action="" method="post" autocomplete="off" id="datasheet">
		...
		<?php
			...
			if($po['isdata'] == true) {
				...
				$isSql = "SELECT * FROM {$tbl} WHERE deletedata=0".$keywords;
				$wgt_iou = idget_data($isSql);
				...
				$isjp = "SELECT * FROM {$tbL151} WHERE subject='{$iou_no}' AND approval_type='IOU'";
				$jpL = idget_data($isjp);
										...
										if($jpL[0]['user_one']) {
											...
											if($userSignedIn != $jpL[0]['user_one']) {
												...
												if($jpL[0]['approval_one'] == 0) {
													?>
														<select name="signpr" id="signpr" onchange="signPr('<?php echo $iou_no; ?>',this.id,1)">
											...
											if($userSignedIn != $jpL[0]['user_two']) {
												...
												if($jpL[0]['approval_two'] == 0) {
													if($jpL[0]['approval_one'] == 1) {
														?>
															<select name="signpr" id="signpr" onchange="signPr('<?php echo $iou_no; ?>',this.id,2)">
																<option value="" selected>Choose to sign</option>
																<option value="1">Approve</option>
																<option value="2">On Hold</option>
																<option value="3">Reject</option>
														...
														?>
															<select name="signpr" id="signpr" onchange="signPr('<?php echo $iou_no; ?>',this.id,2)" disabled="disabled">
											...
											if($userSignedIn != $jpL[0]['user_three']) {
												...
												if($jpL[0]['approval_three'] == 0) {
													if($jpL[0]['approval_two'] == 1) {
														?>
															<select name="signpr" id="signpr" onchange="signPr('<?php echo $iou_no; ?>',this.id,3)">
																<option value="" selected>Choose to sign</option>
																<option value="1">Approve</option>
																<option value="2">On Hold</option>
																<option value="3">Reject</option>
														...
														?>
															<select name="signpr" id="signpr" onchange="signPr('<?php echo $iou_no; ?>',this.id,3)" disabled="disabled">
																<option value="0" selected>Signatory Locked</option>
															</select>
														<?php
													}
											...
											if($userSignedIn != $jpL[0]['user_four']) {
												...
												if($jpL[0]['approval_four'] == 0) {
													if($jpL[0]['approval_three'] == 1) {
														?>
															<select name="signpr" id="signpr" onchange="signPr('<?php echo $iou_no; ?>',this.id,4)">
																<option value="" selected>Choose to sign</option>
																<option value="1">Approve</option>
																<option value="2">On Hold</option>
																<option value="3">Reject</option>
														...
														?>
															<select name="signpr" id="signpr" onchange="signPr('<?php echo $iou_no; ?>',this.id,4)" disabled="disabled">
																<option value="0" selected>Signatory Locked</option>
															</select>
														<?php
													}
											...
											if($userSignedIn != $jpL[0]['user_five']) {
												...
												if($jpL[0]['approval_five'] == 0) {
													if($jpL[0]['approval_four'] == 1) {
														?>
															<select name="signpr" id="signpr" onchange="signPr('<?php echo $iou_no; ?>',this.id,5)">
																<option value="" selected>Choose to sign</option>
																<option value="1">Approve</option>
																<option value="2">On Hold</option>
																<option value="3">Reject</option>
														...
														?>
															<select name="signpr" id="signpr" onchange="signPr('<?php echo $iou_no; ?>',this.id,5)" disabled="disabled">
																<option value="0" selected>Signatory Locked</option>
															</select>
														<?php
													}
					...
					<div class="x-scroll bottom-pull-10">
						<div class="nc-width-100">
							<table cellspacing="0" cellpadding="0">
								...
								<tr>
									<td class="right-pull-10 left-pull-10"><?php echo $wgt_iou[0]['detail']; ?></td>
									<td class="right-pull-10 left-pull-10"><?php echo number_format($wgt_iou[0]['amount']); ?></td>
									<td class="right-pull-10 left-pull-10"><?php echo date($nth_dfn,strtotime($wgt_iou[0]['iou_date'])); ?></td>
								</tr>
				...
				?>
					...
					<div class="block-element" align="center">
						...
						<h3 class="xlarge nobold">You may need to go to the iou receipt section to complete the transaction</h3>
					</div>
				<?php
			}
		?>
...
<script>
	...
	function signPr(order,id,level) {
		...
		if(wgets.value != '' && wgets.value !== null) {
			...
			vhtml += '<input type="hidden" name="uri" value="apply-iou-approval">';
