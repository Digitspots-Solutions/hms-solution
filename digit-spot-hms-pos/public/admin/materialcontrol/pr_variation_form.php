<?php
	...
	$query_pr = "deletedata=0 AND receipt_status='Pending' AND var_status = 0 AND order_status='Approved' AND (pr_status='IOU Approved' OR pr_status='Payment Approved') GROUP BY order_number";
	...
	<div class="cs-width-350 float-right">
		<form action="" method="post">
			...
			<span class="cs-width-200 float-left">
				<input type="text" name="prvar" id="prvar" placeholder="Search for PR">
			</span>
				...
				<?php
					if($pr_state['isdata'] == true) {
						...
						if(isset($_POST['varbutton']) && !empty($_POST['prvar'])) {
							...
							$isSql = "SELECT * FROM {$tbl} WHERE order_number='{$_POST['prvar']}' AND receipt_status='Pending' AND var_status = 0 AND order_status='Approved' AND (pr_status='IOU Approved' OR pr_status='Payment Approved') AND deletedata=0 GROUP BY order_number ORDER BY id DESC";
						} else {
							/*if(isset($_SESSION['prvar'])) {
								$isSql = "SELECT * FROM {$tbl} WHERE order_number='{$_SESSION['prvar']}' AND receipt_status='Pending' AND var_status = 0 AND order_status='Approved' AND (pr_status='IOU Approved' OR pr_status='Payment Approved') AND deletedata=0 GROUP BY order_number ORDER BY id DESC";
							} else {
								$isSql = "SELECT * FROM {$tbl} WHERE receipt_status='Pending' AND var_status = 0 AND order_status='Approved' AND (pr_status='IOU Approved' OR pr_status='Payment Approved') AND deletedata=0 GROUP BY order_number ORDER BY id DESC";
							}*/
							...
							foreach($wgt_pr as $key => $val) {
								...
								?>
									<tr>
										...
										<td class="right-pull-10 left-pull-10 blue-font anchor" onclick="jsPrint('<?php echo $val['order_number']; ?>')"><?php echo $val['order_number']; ?></td>
...
<?php
	...
	if(isset($_GET['nopops'])) { if(isset($_GET['var'])) { unset($_GET['var']); unset($_SESSION['prvar']); } }

	if(isset($_GET['var']) && !empty($_GET['var'])) {
		$this_var = explode('/', $_GET['var']);
		?>
			<div class="fx-position-stick fscr zind-2 txp3-black y-scroll xfadeout motion" align="center">
											...
											foreach($wgt_vpr as $key => $val) {
												...
												?>
													<tr>
														...
														<td class="top-pull-5 bottom-pull-5 right-pull-10 left-pull-10"><input type="text" name="qtydiff[]" id="qtyd<?php echo $numb; ?>" placeholder="Auto?" class="qtydf" readonly required></td>
														...
														<td class="top-pull-5 bottom-pull-5 right-pull-10 left-pull-10"><input type="number" min="1" step=".01" name="mktprice[]" id="mkt<?php echo $numb; ?>" placeholder="Enter here?" class="mktprice" onblur="wgetPvar(<?php echo $numb; ?>)" required></td>
														<td class="top-pull-5 bottom-pull-5 right-pull-10 left-pull-10"><input type="number" min="1" step=".01" name="pricediff[]" id="pricedf<?php echo $numb; ?>" placeholder="Auto?" class="pricedf" readonly required></td>
													</tr>
												<?php
