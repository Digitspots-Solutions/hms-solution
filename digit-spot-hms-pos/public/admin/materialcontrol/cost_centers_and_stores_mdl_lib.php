...
<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		...
		&nbsp; Note: create new store outlet by clicking <u>new store outlet</u> button and fill the necessary information
 	</span>
...
<?php
	...
	$get_dept = '<option value="0">Not Apply</option>';
	...
	$get_fs_parent_store = ""; $limitPr = 0;
	foreach($parent_store as $id => $val) { if($limitPr == 2) { break; } $get_fs_parent_store .= '<option value="'.$id.'">'.$val.'</option>'; $limitPr += 1; }
	...
	{
		createDatabasetable($var_tbl_118); //create a table for this post
		...
		$insert_constrain = "";
		$data_inserted = mysqli_data_insert($tbL123,$insert_dataproperty,$insert_constrain);
		...
		{
			$store_number = "STORE".$mysqli_id;
			$store_datasets = array("store_number"=>$store_number);
			$store_query = array("id"=>$mysqli_id);
			$data_updated = mysqli_data_update($tbL123,$store_datasets,$store_query);

			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Create new store outlet","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
				...
				<div class="nc-width-40">
					<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
						<div class="bottom-push-20 alignlt">
							<h3 class="nomargin">Creating New Store Outlet</h3>
						</div>
						<div class="block-element">
							...
							<span class="block-element bottom-push-15">
								<small class="block-element bottom-push-5 left-pull-5 dark-grey-font alignlt">Store Type <b class="red-font">*</b></small>
								<select name="fieldset3" id="fieldset3" required="required" onchange="pSt()">
									<option value="" selected="selected">Select Type</option>
									<?php echo $get_store_type; ?>
								</select>
							</span>
							<span class="block-element bottom-push-15">
								<small class="block-element bottom-push-5 left-pull-5 dark-grey-font alignlt">Parent Store <b class="red-font">*</b></small>
								<select name="fieldset4" id="fieldset4" required="required">
									<option value="" selected="selected"></option>
								</select>
							</span>
							<span class="block-element bottom-push-15">
								<small class="block-element bottom-push-5 left-pull-5 dark-grey-font alignlt">Address</small>
								<textarea name="fieldset5" id="fieldset5" placeholder="Enter address"></textarea>
							</span>
							<span class="block-element bottom-push-15">
								<small class="block-element bottom-push-5 left-pull-5 dark-grey-font alignlt">Department <b class="red-font">*</b></small>
	...
	{
		$thproperty = array("noth","store name","description","type","parent store","address","department","status","noth","enoth");
		...
		foreach($dataCollect as $theader => $tdata)
		{
			...
			$htmlresult .= '<td width="150px" align="center" class="box-border-thick-right"><a href="javascript:void(0)" class="royal-blue-font" title="Store Details" onclick="get_item_detail('.$dataid.')"><b>'.$tdata["store_name"].'</b></a></td>';
			...
			$htmlresult .= '<td width="150px" align="center" class="box-border-thick-right">'.$this_parent_store.'</td>';
			$htmlresult .= '<td width="200px" align="center" class="box-border-thick-right">'.$tdata["address"].'</td>';
			$htmlresult .= '<td width="150px" align="center" class="box-border-thick-right">'.$this_department.'</td>';
...
<script type="text/javascript">
	...
	function pSt(opt1,opt2) {
		...
		if(eval(st.value) == 1 || eval(st.value) == 5) { fget_ps += '<?php echo $get_fs_parent_store; ?>'; }
