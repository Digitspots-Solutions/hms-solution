<?php
	...
	$inbox_link_mod = array("apply-pr-approval","apply-pr-iou-approval","apply-iou-approval","apply-pr-var-approval","apply-item-request-approval","apply-item-transfer-approval");
	...
	if($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		#turn inbox to archive data if signed

		if(in_array($uri, $inbox_link_mod) && isset($_SESSION['inboxId']) && isset($_SESSION['authenticate_id'])) {

			$pst_query = "id={$_SESSION['inboxId']}";
			$pst_field = "archivedata=1";

			mysqli_data_update($tbL104,$pst_field,$pst_query);
			unset($_SESSION['inboxId']);
		}
		...
		if($uri === 'adjust-stock') {
			...
			if($get_pr_job_level == $level && $signatory == 1) { 
				...
				if(is_array($get_stock_var)) {
					...
					foreach($get_stock_var as $key => $val) {
						...
						if(!empty($val['qty_bought']) && $val['qty_bought'] > 0) {
						...
						if(!empty($val['market_price']) && $val['market_price'] > 0) {
							$for_itcc_query .= "itemid='{$val['itemid']}' AND costprice='{$val['market_price']}';";
							$for_itcc_pst .= "itemid='{$val['itemid']}',costprice='{$val['market_price']}',userid={$userSignedIn},datelogged='{$server_get_date}',timelogged='{$server_get_time}';";
						}
					...
					#update item cost center

					if(!empty($for_itcc_pst)) {

						$pst_query = $for_itcc_query;
						$pst_field = $for_itcc_pst;
			...
			if(is_array($users) && count($users) > 0) {
				...
				$pst_field = "first_approval='{$userSignedIn}',var_status=1,var_approval='No'";
		...
		} elseif($uri === 'update-stock-variance') {
			...
			for($r=0; $r < count($item); $r++) {
				$total_amount = $qty_bought[$r] * $mkt_price[$r];
				$pst_query .= "order_number='{$order_number}' AND itemid={$item[$r]};";
				$pst_field .= "qty_bought='{$qty_bought[$r]}',qty_diff='{$qty_diff[$r]}',market_price='{$mkt_price[$r]}',price_diff='{$price_diff[$r]}',total_amount='{$total_amount}',userid={$userSignedIn},datelogged='{$server_get_date}',timelogged='{$server_get_time}';";
				$total_amount = 0;
			}

			$entry = "update";
			$params['body'] = "PR variation updated successfully";
			$params['tbls'] = $mtbL20;
			$params['datasets'] = $pst_field;
			$params['constrains'] = $pst_query;
			$params['loop'] = 2;
			...
			$pr_var_data = "SELECT * FROM $mtbL8 WHERE order_number='{$orderno}' AND var_approval='Yes' AND receipt_status='Pending' AND order_net_amount > 0 AND deletedata=0"; $get_pr_var_data = idget_data($pr_var_data);
			...
			if(is_array($get_pr_var_data)) {
				...
				foreach($get_pr_var_data as $key => $val) {
					...
					$for_pr_pst_field .= "second_approval='{$userSignedIn}',delivery_date='{$server_get_date}',receipt_status='Received',invoice_number='{$invoice_number}';";
				}
		...
		} elseif($uri === 'apply-pr-instant-receive') {
			...
			if(is_array($get_pr_var_data)) {
				...
				foreach($get_pr_var_data as $key => $val) {
					...
					if($val['store'] > 0) {
						...
						$for_itcc_query .= "itemid='{$val['itemid']}' AND costprice='{$val['unitprice']}';";
						$for_itcc_pst .= "itemid='{$val['itemid']}',costprice='{$val['unitprice']}',userid={$userSignedIn},datelogged='{$server_get_date}',timelogged='{$server_get_time}';";
				...
				if(is_array($get_pr_var_data)) {
					...
					foreach($get_pr_var_data as $key => $val) {
						...
						if($val['store'] == 0) {
							...
							if($is_item_exist['isdata'] == true) {
								$csql = "SELECT * FROM {$tbL16} WHERE ".$chk_item;
								$cdata = idget_data($csql);
								
								$is_item_bal = $cdata[0]['balance'];
			...
			if(isset($job_order) && !empty($job_order)) {
				$pst_field = "iou_no='{$iou_no}',pr_no='{$job_order}',pr_type='Job Order',amount='{$amount}',datelogged='{$server_get_date}',timelogged='{$server_get_time}'";
			} else {
				$pst_field = "iou_no='{$iou_no}',pr_type='Manual',amount='{$amount}',datelogged='{$server_get_date}',timelogged='{$server_get_time}'";
			}
			...
			$pst_query = "request_number='{$request_number}'";
			$pst_field = "request_number='{$request_number}',remark='{$remark}',datelogged='{$server_get_date}',timelogged='{$server_get_time}'";

			$entry = "insert";
			$params['body'] = "";
			$params['tbls'] = "ir_remark_tbl";
			$params['datasets'] = $pst_field;
			$params['constrains'] = $pst_query;
			$params['loop'] = 1;
			...
			$pst_query = "iou_no='{$iou_no}'";
			$pst_field = "iou_no='{$iou_no}',pr_no='{$orderno}',pr_type='Internal Purchase',datelogged='{$server_get_date}',timelogged='{$server_get_time}'";
			...
			for($f=0; $f < count($fb); $f++) {
				...
				$ffr = mysqli_data_exist('fd_analysis_tbl',$pst_query);

				if($ffr['isdata'] == true) {
					$pst_field = "amount='{$val}'";
					mysqli_data_update('fd_analysis_tbl',$pst_field,$pst_query);
				} else {
					...
					mysqli_data_insert('fd_analysis_tbl',$pst_field,$pst_query);
				}
			...
			for($b=0; $b < count($bsb); $b++) {
				...
				$bbr = mysqli_data_exist('fd_analysis_tbl',$pst_query);
			...
			for($s=0; $s < count($outlets); $s++) {
				...
				$olt = mysqli_data_exist('outlet_food_analysis_tbl',$pst_query);

				if($olt['isdata'] == true) {
					$pst_field = "cover='{$cval}',food='{$fval}'";
					mysqli_data_update('outlet_food_analysis_tbl',$pst_field,$pst_query);
				} else {
					...
					mysqli_data_insert('outlet_food_analysis_tbl',$pst_field,$pst_query);
				}
			...
			$saynotify = 1;
			$post_header = "Notification";
			$post_message = "FF information processed successfully";
	...
	if(isset($_GET['curi']) && !empty($_GET['curi'])) {
		...
		if($curi === 'delete-record') {
			...
			if($pr2IOU['isdata'] == true) {
				...
				$stock_var2 = "SELECT * FROM $mtbL8 WHERE order_number='{$orderno}'";
				$get_stock_var2 = idget_data($stock_var2);

				$pst_query = "";
				$pst_field = "";

				foreach($get_stock_var2 as $key => $val) {
					$pst_query .= "itemid='{$val['itemid']}' AND costprice='{$val['unitprice']}';";
					$pst_field .= "itemid='{$val['itemid']}',costprice='{$val['unitprice']}',userid={$userSignedIn},datelogged='{$server_get_date}',timelogged='{$server_get_time}';";
				}
