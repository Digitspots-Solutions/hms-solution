<?php
	...
	if(isset($_GET['storeid']) && $_GET['storeid'] > 0) {
		...
		$keywords = "storeid={$_GET['storeid']} AND deletedata=0 AND acceptance=0 AND status IN({$ustat}) GROUP BY storeid,request_number";
	} else {
		$keywords = "deletedata=0 AND acceptance=0 AND status IN('Reviewing','Under Approval','Ready to Disburse') GROUP BY storeid,request_number";
	}
												...
												?>
													<tr class="">
														<td colspan="11" align="left" class="top-pull-5">
															...
															<input type="button" value=" Disburse Stock " class="pads7 blue-white-state rounded-button nc-width-15 left-push-20 anchor" onclick="disburseStock('fr-<?php echo $val['request_number']; ?>','<?php echo $val['request_number']; ?>')" title="Click here to disburse stock to this store">
														</td>
													</tr>
												<?php
											}
...
<script>
	...
	function disburseStock(frm,requestno) {
