<?php
	...
	$query_outlets = "SELECT * FROM {$tbL14} WHERE status IN('Active') AND deletedata=0";
	$for_outlets = html_db_select($query_outlets,'id','posname');
...
<div class="white-theme top-pull-7 right-pull-20 left-pull-20 bottom-push-10">
	...
	&nbsp; Note: Here you see the item details transferred from store
	</span>
	...
	<div class="nc-width-100">
		<form action="" method="post" autocomplete="off" id="reportform" class="nomargin nopads">
			...
			<span class="ln-display-box float-left cs-width-200 top-pull-7 right-push-20">
				...
				<select name="outlets" id="outlets" class="nopads no-back-black">
					<option value="" selected>All</option>
					<?php echo $for_outlets; ?>
				</select>
			</span>
					...
					if(isset($_POST['reporter']) && $_POST['reporter'] == 'stock-issue-reports') {
						...
						?>
							...
							<div class="cs-width-700 margin-auto-ct alignct">
								...
								<h3 class="large nobold nomargin">Items Issued from Store (Between <?php echo date('d/m/y',strtotime($_POST['startdate'])); ?> And <?php echo date('d/m/y',strtotime($_POST['enddate'])); ?>)</h3>
						...
						if(isset($_POST['outlets']) && !empty($_POST['outlets'])) {
							$keywords .= " AND to_posid={$_POST['outlets']}";
							$oot = " AND to_posid={$_POST['outlets']}";
						} else {
							$oot = "";
						}
						...
						$sql = "SELECT from_posid,transfer_status FROM {$xtbl} WHERE transfer_status IN('Transfer Completed') AND deletedata=0".$keywords." GROUP BY from_posid";
						...
						if(is_array($datagroup)) {
							...
							foreach($datagroup as $key => $val) {
								...
								$store_name = idget_name($val['from_posid'],'store_name',$tbL123);
								...
								$sql2 = "SELECT to_posid FROM {$xtbl} WHERE from_posid={$val['from_posid']}".$oot." AND transfer_status IN('Transfer Completed') AND deletedata=0".$ddOpt." GROUP BY to_posid";
								$datagroup2 = idget_data($sql2);
							
								?>
									<h2 class="large nobold default-text-font-bold bottom-pull-5"><?php echo $store_name.$stat_name; ?></h2>
								<?php

								foreach($datagroup2 as $key2 => $val2) {

									$sql3 = "SELECT * FROM {$xtbl} WHERE from_posid={$val['from_posid']} AND to_posid={$val2['to_posid']} AND transfer_status IN('Transfer Completed') AND deletedata=0".$ddOpt;
									$datagroup3 = idget_data($sql3);

									$get_pos_name = idget_name($val2['to_posid'],'posname',$tbL14);

									?>
										<h3 class="large nobold default-text-font-bold bottom-pull-5"><?php echo $get_pos_name; ?></h3>
										...
										<table cellpadding="3" cellspacing="0" border="1">
											<tr>
												...
												<td class="alignct default-text-font-bold">Stock Bal.</td>
											...
											<?php
												...
												foreach($datagroup3 as $key3 => $val3) {
													
													$num += 1;

													idget_global($val3['itemid'],$var_item);
													$get_su = arrayget_key($uoms,$val3['uom']);
													$expiry_state = idget_name($val3['itemid'],'isexpire',$mtbL5);
													$expiry_date = idget_name($val3['itemid'],'expiry_date',$mtbL5);

													$expiry_date_fr = date('d-m-y',strtotime($expiry_date));
													$get_expiry_stat = ($expiry_state == 'Yes') ? "Expiring - {$expiry_date_fr}" : "Never Expire";

													$lsp = "SELECT * FROM {$mtbL18} WHERE itemid={$val3['itemid']} AND deletedata=0 ORDER BY id DESC LIMIT 1"; $get_lsp = idget_data($lsp);

													$last_price = $get_lsp[0]['costprice'];
													$cost_amount = $val3['qty_transfer'] * $last_price;

													$stkBal = "SELECT balance FROM {$mtbL19} WHERE storageid={$val['from_posid']} AND itemid={$val3['itemid']} AND deletedata=0 ORDER BY id DESC LIMIT 1";
													$get_stkBal = idget_data($stkBal);
													...
													?>
														<tr>
															...
															<td class="alignct"><?php echo $val3['transfer_number']; ?></td>
															<td class="alignct"><?php echo date('d-m-Y',strtotime($val3['datelogged'])).' '.$val3['timelogged']; ?></td>
															...
															<td class="right-pull-10 left-pull-10 alignlt"><?php echo $val3['qty_transfer'].' '.$get_su; ?></td>
															<td class="right-pull-10 left-pull-10 alignlt"><?php echo $get_stkBal[0]['balance'].' '.$get_su; ?></td>
