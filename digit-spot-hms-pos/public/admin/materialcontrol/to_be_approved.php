<?php
	...
	if(isset($_GET['orderno'])) {
		$keywords = " AND order_number='{$_GET['orderno']}' AND order_status IN('Pending') AND ispr_to_manual IN('No') AND gstat='Confirm'".$toStore;
		unset($_GET['orderno']);
	} else {
		$keywords = " AND order_status IN('Archiver') AND gstat='Confirm'".$toStore;
	}
	...
	$lAr = "SELECT * FROM {$tbl} WHERE order_status IN('Pending') AND ispr_to_manual IN('No') AND gstat='Confirm' AND deletedata=0".$toStore." GROUP BY order_number";
...
<div class="pads30">
	...
	<div class="float-right cs-width-250 box-border-thick sml-rounded-button pads15 alignlt">
		<h4 class="xlarge nobold default-text-font-bold">Pending Approvals</h4>
	...
	<div class="alignlt"><h3 class="xlarge nobold nomargin"><a href="<?php echo $ths_page; ?>" title="Refresh" class="right-push-10"><img src="<?php echo DOMAIN_URL; ?>theme/images/general/refresh.png"></a> Here are list of PRs set for approval</h3></div>
			...
			if($po['isdata'] == true) {
								...
								<p class="alignrt bottom-pull-20">
									<?php
										if($val['gstat'] == 'Pending') {
											?>
												<!--<a href="javascript:void(0)" class="blue-font box-border-thick top-pull-7 right-pull-20 bottom-pull-7 left-pull-20 rounded-button white-theme obj-light-shadow right-push-10" onclick="jsPrint(<?php //echo $val['supplierid']; ?>,<?php //echo $val['order_number']; ?>)">Print <b class="fas fa-print left-push-5"></b></a>-->
											...
											$isjp = "SELECT * FROM {$tbL151} WHERE subject='{$val['order_number']}' AND approval_type='PR' AND approval_status IN('Pending')"; $jpL = idget_data($isjp);
											...
											if($val['order_status'] == 'Approved') {
												...
												if($val['pr_status'] == 'IOU') {
													?>
														* Set for IOU Approval
													<?php
												} elseif($val['pr_status'] == 'Payment Inview') {
													?>
														<a href="javascript:void(0)" class="blue-font box-border-thick top-pull-7 right-pull-20 bottom-pull-7 left-pull-20 rounded-button white-theme obj-light-shadow right-push-10" onclick="">Print LPO <b class="fas fa-print left-push-5"></b></a>
				...
				?>
					...
					<div class="block-element" align="center">
						<h3 class="large nobold dark-grey-font">See your pending approval orders here. Choose from search list</h3>
					</div>
				<?php
			}
		?>
...
<script>
			...
			if(document.getElementById('table-form')) {
				...
				function wgtpop(response) {
					...
					for(i=0; i<data.length; i++) {
						...
						vhtml += '<td class="nunito-semibold right-pull-10 left-pull-10"><input type="number" min="1" name="quantity[]" id="quantity'+nrow+'" placeholder="Enter quantity?" value="'+data[i].qty_ordered+'" class="nopads no-back-black quantity" onkeypress="nextRow(this.id,event)" onblur="lastRow(this.id)" required></td>';
	...
	/*function jsPrint(order,batch) {
		...
		inframe.src = curl+'workspace/wks_ini.php?wget='+wgets+'&param=print-po';
	}*/
