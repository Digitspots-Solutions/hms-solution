...
<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		...
		&nbsp; Note: create new item sub-category by clicking <u>new sub-category</u> button
 	</span>
...
<?php
	...
	if(isset($_POST['submitbutton']))
	{
		...
		$insert_dataproperty = array("categoryid"=>$fieldset1,"subcategory"=>ucwords(strtolower($fieldset2)),"detail"=>$fieldset3);
		$insert_constrain = "";
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Create new stock sub-category","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
			...
			<div class="block-element box-border-thick-bottom bottom-pull-30 bottom-push-30" align="center">
				<div class="nc-width-40">
					<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
						...
						<span class="block-element bottom-push-10">
							<input type="text" name="fieldset2" id="fieldset2" placeholder="Enter name" required="required">
						</span>
						<span class="block-element bottom-push-10">
							<textarea name="fieldset3" id="fieldset3" placeholder="Enter description" required="required"></textarea>
						</span>
						<span class="block-element">
							<input type="hidden" name="fieldset4" id="fieldset4" value="0" required="required">
						</span>
	...
	if((isset($_POST['statusbutton']) && isset($_POST['checkers'])) && (isset($_POST['cstatus']) && !empty($_POST['cstatus'])))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$cstat = mysqli_data_update($tbL116,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Change stock sub-category status","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['editbutton']))
	{
		...
		$data_inserted = mysqli_data_update($tbL116,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Edit stock sub-category details","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['deletebutton']) && isset($_POST['checkers']))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$usr_key = array("id"=>$fkey);
			$del = mysqli_data_update($tbL116,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Remove from stock sub-category list","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if((isset($_POST['searchbutton'])) && (isset($_POST['search']) && !empty($_POST['search']))) {
		$keywords=" AND subcategory LIKE '".escape_data($_POST['search'])."%'";
	} else { 
		$keywords="";
	}
	...
	$dataproperty = "id,categoryid,subcategory,detail,status";
	$constrain = array("deletedata"=>0);
	$row = "array";

	$dataCollect = mysqli_data_fetch($tbL116,$dataproperty,$constrain,$row);

	if(is_array($dataCollect))
	{
		$thproperty = array("noth","sub-category","detail","noth","enoth");
		...
		$num=$pgstart; $g=""; $dataid=""; $category_name="";
		...
		foreach($dataCollect as $theader => $tdata)
		{
			...
			$htmlresult .= '<td width="200px" align="center" class="box-border-thick-right">'.$tdata["subcategory"].'</td>';
			...
			if((isset($_GET['edit']) && $_GET['edit'] >= 1) && ($_GET['edit'] == $dataid))
			{
				...
				$htmlresult .= '<h4 class="large blue-font">Updating '.$tdata["subcategory"].'</h4><br>';
				...
				$htmlresult .= '<input type="text" name="fieldset2" id="fieldset2" placeholder="Enter name" value="'.$tdata["subcategory"].'" required="required">';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
	...
	$additionalQuery = "";
	mysqli_data_check($tbL116,'(*)',$constrain);
