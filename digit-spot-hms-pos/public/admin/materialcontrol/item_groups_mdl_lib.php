...
<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		...
		&nbsp; Note: create item group by clicking <u>new group name</u> button
 	</span>
...
<?php
	...
	if(isset($_POST['submitbutton']))
	{
		...
		$insert_dataproperty = array("categoryid"=>$fieldset1,"subcategoryid"=>$fieldset2,"groupname"=>ucwords(strtolower($fieldset3)),"detail"=>$fieldset4);
		$insert_constrain = "";
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Create new stock item group name","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
				...
				<div class="nc-width-40">
					<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
						<div class="bottom-push-20 alignlt">
							<h3 class="nomargin">Creating New Sub-Category</h3>
						</div>
						<span class="block-element bottom-push-10">
							<select name="fieldset1" id="fieldset1" required="required" onchange="getdata('fieldset2','eget-item-subcategory-list','fieldset1','dropbox');">
						...
						<span class="block-element bottom-push-10">
							<input type="text" name="fieldset3" id="fieldset3" placeholder="Enter name" required="required">
						</span>
						<span class="block-element bottom-push-10">
							<textarea name="fieldset4" id="fieldset4" placeholder="Enter description" required="required"></textarea>
						</span>
	...
	if((isset($_POST['statusbutton']) && isset($_POST['checkers'])) && (isset($_POST['cstatus']) && !empty($_POST['cstatus'])))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$cstat = mysqli_data_update($tbL117,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Change stock item group status","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['editbutton']))
	{
		...
		$data_inserted = mysqli_data_update($tbL117,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Edit stock item group details","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['deletebutton']) && isset($_POST['checkers']))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$usr_key = array("id"=>$fkey);
			$del = mysqli_data_update($tbL117,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Remove from stock item group list","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if((isset($_POST['searchbutton'])) && (isset($_POST['search']) && !empty($_POST['search']))) {
		$keywords=" AND groupname LIKE '".escape_data($_POST['search'])."%'";
	} else { 
		$keywords="";
	}
	...
	$dataproperty = "id,categoryid,subcategoryid,groupname,detail,status";
	$constrain = array("deletedata"=>0);
	$row = "array";

	$dataCollect = mysqli_data_fetch($tbL117,$dataproperty,$constrain,$row);

	if(is_array($dataCollect))
	{
		$thproperty = array("noth","name","detail","noth","enoth");
		...
		$num=$pgstart; $g=""; $dataid=""; $category_name=""; $subcategory_name="";
		...
		foreach($dataCollect as $theader => $tdata)
		{
			...
			$htmlresult .= '<td width="200px" align="center" class="box-border-thick-right">'.$tdata["groupname"].'</td>';
			...
			if((isset($_GET['edit']) && $_GET['edit'] >= 1) && ($_GET['edit'] == $dataid))
			{
				...
				$category_name = idget_data($tbL115,$tdata["categoryid"],'category');
				$subcategory_name = idget_data($tbL116,$tdata["subcategoryid"],'subcategory');
				$get_active_categories = select_dt_fetch('status','Active',$tbL115,'id','category');
				$get_active_subcategories = select_dt_fetch('status','Active',$tbL116,'id','subcategory');

				$writeDataIn="'fieldset2'"; $selectOpt="'eget-item-subcategory-list'"; $getDataKeyFrom="'fieldset1'"; $formObj="'dropbox'";

				$htmlresult .= '<tr>';
				$htmlresult .= '<td colspan="30">';
				$htmlresult .= '<div id="tr" class="block-element grey-1-theme pads30">';
				$htmlresult .= $update_result;
				$htmlresult .= '<h4 class="large blue-font">Updating '.$tdata["groupname"].'</h4><br>';
				...
				$htmlresult .= '<select name="fieldset1" id="fieldset1" required="required" onclick="getdata('.$writeDataIn.','.$selectOpt.','.$getDataKeyFrom.','.$formObj.')">';
				$htmlresult .= '<option value="'.$tdata["categoryid"].'">'.$category_name.'</option>';
				...
				$htmlresult .= '<option value="'.$tdata["subcategoryid"].'">'.$subcategory_name.'</option>';
				$htmlresult .= $get_active_subcategories;
				$htmlresult .= '</select>';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Item Group</small>';
				$htmlresult .= '<input type="text" name="fieldset3" id="fieldset3" placeholder="Enter name" value="'.$tdata["groupname"].'" required="required">';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Description</small>';
				$htmlresult .= '<textarea name="fieldset4" id="fieldset4" placeholder="Enter description" required="required">'.$tdata["detail"].'</textarea>';
				$htmlresult .= '</span>';
				$htmlresult .= '</div>';
	...
	$additionalQuery = "";
	mysqli_data_check($tbL117,'(*)',$constrain);
