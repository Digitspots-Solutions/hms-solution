<?php
	...
	$mgh_get_vat = idget_name(1,'vat',$tbL109);
	$mgh_get_service_charge = idget_name(1,'service_charge',$tbL109);
	$mgh_get_consumption_tax = idget_name(1,'consumption_tax',$tbL109);

	$ff=0;

	if(isset($_GET['trdate']) && !empty($_GET['trdate'])) { $ff=1; $tr_date = $_GET['trdate']; }
	else { $ff=0; $tr_date = date('Y-m-d',strtotime($server_get_date . ' -1 days'));  }

	$wget_outlet_sql = "SELECT posid, SUM(amount) AS totalbillamt4outlet FROM {$tbL99} WHERE main_category IN(1) AND billtype IN(1,2,4,5) AND datelogged='{$tr_date}' AND isreversed=0 AND deletedata=0 GROUP BY posid";
	$wget_outlet =  idget_data($wget_outlet_sql);
			...
			<div class="sided-box pads15 box-border-thick-bottom alignlt">
				<table cellpadding="0" cellspacing="5">
					...
					<?php
						...
						$posname = ""; $fflash = ""; $wget_outlet_covers = "";
						$grand_total_covers = 0; $grand_total_foods = 0;

						if(is_array($wget_outlet)):
							
							$vat = 0; $consumption = 0; $service = 0; $notreversed = "";
							...
							foreach($wget_outlet as $key => $val):
								...
								if($fflash == 'Yes'):
									...
									$notreversed .= $val['posid'].',';
								
									$wget_outlet_covers_sql = "SELECT SUM(cover) AS 'totalcover4outlet' FROM {$tbL100} WHERE posid={$val['posid']} AND billtype IN(1,2,4,5) AND datelogged='{$tr_date}'";
									$wget_outlet_covers = idget_data($wget_outlet_covers_sql);
									...
									if($pos_ff[0]['food'] > 0) {
										$pos_amount = $pos_ff[0]['food'];
									} else {
										$tax = ($mgh_get_vat / 100) * $val['totalbillamt4outlet'];
										$consumption = ($mgh_get_consumption_tax / 100) * $val['totalbillamt4outlet'];
										$service = ($mgh_get_service_charge / 100) * $val['totalbillamt4outlet'];
										$pos_amount = $val['totalbillamt4outlet'] - ($tax + $consumption + $service);
									}

									if($pos_ff[0]['cover'] > 0) { $pos_covers = $pos_ff[0]['cover']; }
									else { $pos_covers = $wget_outlet_covers[0]['totalcover4outlet']; }
							...
							#remove any already reversed outlets from main account in food flash

							if(isset($ff) && $ff == 1):
								$notreversedFx = substr_replace($notreversed,'',-1,1);
								$pos_na = "SELECT id,pos FROM outlet_food_analysis_tbl WHERE pos NOT IN({$notreversedFx}) AND transaction_date='{$tr_date}'"; $show_pos_na = idget_data($pos_na);

								if(is_array($show_pos_na) && count($show_pos_na) > 0):
									foreach($show_pos_na as $key => $val):
										$query = "id={$val['id']}";
										trash_record('outlet_food_analysis_tbl',$query);
									endforeach;
								endif;
							endif;
