<div class="alignlt left-pull-30 bottom-push-30"><h3 class="large nobold nomargin">Here are the list of awaiting items going into bad & damage repository. Please acknowledge your consent</h3></div>
<div class="pads30">
	...
	<?php
		...
		$get_requestno = isset($_GET['bnd']) ? $_GET['bnd'] : "";
		
		$isjp = "SELECT * FROM {$tbL151} WHERE subject='{$get_requestno}' AND approval_type='BND'";
		$jpL = idget_data($isjp);

		if(!empty($get_requestno) && $get_requestno != '') { $keywords = "bnd_number='{$get_requestno}' AND deletedata=0 AND bnd_status IN('Under Approval') GROUP BY bnd_number"; } else { $keywords = "deletedata=0 AND bnd_status IN('Under Approval') GROUP BY bnd_number"; }
		
		$sqldata = "SELECT * FROM {$tbL157} WHERE ".$keywords;
		...
		if(is_array($wgt_ir)) {
			...
			foreach($wgt_ir as $key => $val) {
				...
				$sqldata2 = "SELECT * FROM {$tbL157} WHERE bnd_number='{$val['bnd_number']}' AND deletedata=0 AND bnd_status IN('Under Approval')"; $wgt_ir2 = idget_data($sqldata2);
				...
				?>
					<form action="" method="post" autocomplete="off">
						...
						<h3 class="large nobold nunito-bold alignlt">+ Bad & Damage Item #<?php echo $val['bnd_number']; ?></h3>
						<!--<input type="hidden" name="uri" value="edit-item-transfer">-->
						<input type="hidden" name="requestno" value="<?php echo $val['bnd_number']; ?>">
							...
							<div class="nc-width-100">
								<table cellspacing="0" cellpadding="0">
									<tr>
										...
										<td class="default-text-font-bold right-pull-10 left-pull-10">Cost</td>
										<td class="default-text-font-bold right-pull-10 left-pull-10">Storekeeper</td>
									</tr>
									<?php
										$numb = 0;
										...
										foreach($wgt_ir2 as $key2 => $val2) {
											...
											?>
												<tr>
													...
													<td class="right-pull-10 left-pull-10 alignlt"><?php echo $val2['stock'].' '.$get_su; ?></td>
													<td class="right-pull-10 left-pull-10"><?php echo number_format($val2['total_cost']); ?></td>
...
<script>
	...
	function signPr(nos,id,level) {
		...
		if(wgets.value != '' && wgets.value !== null) {
			...
			vhtml += '<input type="hidden" name="uri" value="apply-bnd-approval">';
