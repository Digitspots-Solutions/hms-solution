...
<div id="section-to-print">
	...
	<?php
		if(isset($_POST['submitbutton'])) {
			...
			$rm_inclusion = (isset($_POST['inclcheckin']) && !empty($_POST['inclcheckin'])) ? "'CheckedOut','CheckedIn','Swapped','Upgraded','Downgraded'" : "'CheckedOut','Swapped','Upgraded','Downgraded'";

			if($trx_type === 'Bookings') {

				if(isset($_POST['inclcheckin']) && !empty($_POST['inclcheckin'])) {
					$endttr = date('Y').'-12-31';
					$sql_bkg = "SELECT t1.booking_number,t2.customerid,t2.roomid,t2.checkin_date,t2.checkin_time,t2.checkout_date,t2.checkout_time,t2.noofdays,t2.status FROM $tbL130 t1, $tbL127 t2 WHERE t1.booking_number=t2.booking_number AND t1.reservation IN({$bkg_inclusion}) AND t1.bill_to={$_POST['corporates']} AND t1.checkin_date >= '{$_POST['startdate']}' AND t1.checkout_date <= '{$endttr}' AND t1.bill_type IN('Corporate') ORDER BY t2.checkout_date ASC, t2.checkout_time ASC";
				} else {
					$sql_bkg = "SELECT t1.booking_number,t2.customerid,t2.roomid,t2.checkin_date,t2.checkin_time,t2.checkout_date,t2.checkout_time,t2.noofdays,t2.status FROM $tbL130 t1, $tbL127 t2 WHERE t1.booking_number=t2.booking_number AND t1.reservation IN({$bkg_inclusion}) AND t1.bill_to={$_POST['corporates']} AND t1.checkin_date >= '{$_POST['startdate']}' AND t1.checkout_date <= '{$_POST['enddate']}' AND t1.bill_type IN('Corporate') ORDER BY t2.checkout_date ASC, t2.checkout_time ASC";
				}
						...
						<table cellpadding="3" cellspacing="0" border="1">
							...
							<?php
								if(is_array($dataset_bkg)) {
									...
									$total_consumption = 0; $total_room_count = 0;

									foreach($dataset_bkg as $key => $val) {
										
										if(($val['status'] != 'Swapped' && $val['status'] != 'Upgraded' && $val['status'] != 'Downgraded' && strtotime($val['checkout_date']) >= strtotime($val['checkin_date'])) || (($val['status'] == 'Swapped' || $val['status'] == 'Upgraded' || $val['status'] == 'Downgraded') && (strtotime($val['checkout_date']) > strtotime($val['checkin_date'])))) {
											...
											if($consumed > 0) {
												...
												?>
													<tr>
														...
														<td class="alignct"><?php echo $total_consumed[0]['dayuse']; ?></td>
									...
									?>
										<tr>
											...
											<td class="alignlt" colspan="5"><?php echo $total_room_count; ?> Found</td>
				...
				if(isset($_POST['inclcheckin']) && !empty($_POST['inclcheckin'])) {
					$endttr = date('Y').'-12-31';
					$sql_bkg = "SELECT t1.booking_number,t2.customerid,t2.roomid,t2.checkin_date,t2.checkin_time,t2.checkout_date,t2.checkout_time,t2.noofdays,t2.status FROM $tbL130 t1, $tbL127 t2 WHERE t1.booking_number=t2.booking_number AND t1.reservation IN({$bkg_inclusion}) AND t1.bill_to={$_POST['corporates']} AND t2.status IN({$rm_inclusion}) AND t1.checkin_date >= '{$_POST['startdate']}' AND t1.checkout_date <= '{$endttr}' AND t1.bill_type IN('Corporate') ORDER BY t2.checkout_date ASC, t2.checkout_time ASC";
				} else {
					$sql_bkg = "SELECT t1.booking_number,t2.customerid,t2.roomid,t2.checkin_date,t2.checkin_time,t2.checkout_date,t2.checkout_time,t2.noofdays,t2.status FROM $tbL130 t1, $tbL127 t2 WHERE t1.booking_number=t2.booking_number AND t1.reservation IN({$bkg_inclusion}) AND t1.bill_to={$_POST['corporates']} AND t2.status IN({$rm_inclusion}) AND t1.checkin_date >= '{$_POST['startdate']}' AND t1.checkout_date <= '{$_POST['enddate']}' AND t1.bill_type IN('Corporate') ORDER BY t2.checkout_date ASC, t2.checkout_time ASC";
				}
				...
				?>
								...
								if(is_array($dataset_bkg)) {
									...
									foreach($dataset_bkg as $key => $val) {
										...
										if(($val['status'] != 'Swapped' && $val['status'] != 'Upgraded' && $val['status'] != 'Downgraded' && strtotime($val['checkout_date']) >= strtotime($val['checkin_date'])) || (($val['status'] == 'Swapped' || $val['status'] == 'Upgraded' || $val['status'] == 'Downgraded') && (strtotime($val['checkout_date']) > strtotime($val['checkin_date'])))) {
											...
											$sql_2 = "SELECT SUM(bill_amount) AS totaloutletcharge FROM $tbL100 WHERE billtype IN(2) AND booking_number='{$val['booking_number']}' AND roomid={$val['roomid']} AND biller={$_POST['corporates']} AND status='Completed' AND isreversed=0 AND deletedata=0 AND datelogged >= '{$_POST['startdate']}' AND datelogged <= '{$_POST['enddate']}'";
						...
						<?php
							if(is_array($datagroup)) {
								...
								foreach($datagroup as $key => $val) {
									...
									?>
										<div id="p-<?php echo $val['posid']; ?>" class="bottom-push-20" ondblclick="this.setAttribute('class','noshow')">
					...
					<div class="bottom-push-30" align="center">
						...
						<div id="sccr" class="cs-width-400" ondblclick="chgclass('sccr','noshow')">
							...
							<table cellpadding="3" cellspacing="0" border="1">
								...
								<tr>
									<td class="alignlt">Total Outlets Bill</td>
					...
					<div class="bottom-push-30" align="center">
						...
						<?php
							...
							$a2_queryset = "cspgid={$dataid} AND transaction_type='Credit' AND paymode > 0 AND datelogged >= '{$bizdate}' AND datelogged <= '{$_POST['enddate']}'"; $g_total_pay = mysqli_arithmetic_data($tbL63,$sqlset,$a2_queryset);
							...
							$closing_bal = $g_total_pay - $g_total_consumption;
							//$closing_bal = idget_data($tbL58,$dataid,'creditlimit');

						?>

						<div id="cabo" class="cs-width-500" ondblclick="chgclass('cabo','noshow')">
