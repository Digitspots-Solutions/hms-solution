...
<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		...
		&nbsp; Note: create new gallery by clicking <u>new gallery</u> button
 	</span>
...
<?php
	...
	{
		createDatabasetable($var_tbl_43); //create a table for this post
		...
		$insert_dataproperty = array("thumbnail_image_url"=>$fieldset1,"big_image_url"=>$fieldset2,"detail"=>$fieldset3,"gallery_type"=>$fieldset4);
		$insert_constrain = "";
		$data_inserted = mysqli_data_insert($tbL45,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Create new gallery","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
				...
				<div class="nc-width-40">
					<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
						<div class="bottom-push-20 alignlt">
							<h3 class="nomargin">Creating New Gallery</h3>
						</div>
						<span class="block-element bottom-push-10">
							<input type="url" name="fieldset1" id="fieldset1" placeholder="Enter thumbnail image url" required="required">
						</span>
						<span class="block-element bottom-push-10">
							<input type="url" name="fieldset2" id="fieldset2" placeholder="Enter big image url" required="required">
						</span>
						<span class="block-element bottom-push-10">
							<input type="text" name="fieldset3" id="fieldset3" placeholder="Enter caption" required="required">
						</span>
						<span class="block-element bottom-push-10">
							<select name="fieldset4" id="fieldset4" required="required">
								<option value="" selected="selected">Gallery Type</option>
	...
	if(isset($_POST['editbutton']))
	{
		...
		$insert_dataproperty = array("thumbnail_image_url"=>$fieldset1,"big_image_url"=>$fieldset2,"detail"=>$fieldset3,"gallery_type"=>$fieldset4);
		$insert_constrain = array("id"=>$fieldset5);
		$data_inserted = mysqli_data_update($tbL45,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Edit gallery details","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['deletebutton']) && isset($_POST['checkers']))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$del = mysqli_data_update($tbL45,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Remove from gallery list","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if((isset($_POST['searchbutton'])) && (isset($_POST['search']) && !empty($_POST['search']))) {
		$keywords=" AND gallery_type LIKE '".escape_data($_POST['search'])."%'";
	} else { 
		$keywords="";
	}
	...
	$dataproperty = "id,thumbnail_image_url,big_image_url,detail,gallery_type";
	$constrain = array("deletedata"=>0);
	$row = "array";

	$dataCollect = mysqli_data_fetch($tbL45,$dataproperty,$constrain,$row);

	if(is_array($dataCollect))
	{
		$thproperty = array("noth","gallery type","thumbnail image url","big image url","caption","noth","noth","enoth");
		...
		foreach($dataCollect as $theader => $tdata)
		{
			...
			$htmlresult .= '<td width="100px" align="center" class="box-border-thick-right">'.$tdata["gallery_type"].'</td>';
			$htmlresult .= '<td width="200px" align="center" class="box-border-thick-right">'.$tdata["thumbnail_image_url"].'</td>';
			$htmlresult .= '<td width="200px" align="center" class="box-border-thick-right">'.$tdata["big_image_url"].'</td>';
			...
			$htmlresult .= '<td width="70px" align="center" class="box-border-thick-right"><a href="?logs='.$logs.'&image='.$dataid.'&pg='.$curpage.'&start='.$pgstart.'&limit='.$pglimit.'&#tr" class="blue-font">View Image</a></td>';
			...
			if((isset($_GET['image']) && $_GET['image'] >= 1) && ($_GET['image'] == $dataid))
			{
				...
				$htmlresult .= '<div id="tr" class="block-element white-theme pads30">';
				$htmlresult .= '<h4 class="large">'.$tdata["detail"].'</h4><br>';
				$htmlresult .= '<span class="ln-display-box float-left nc-width-45">';
				$htmlresult .= '<div class="cs-width-250 cs-height-250 noscroll">';
				$htmlresult .= '<img src="'.$tdata["thumbnail_image_url"].'" class="auto-wh">';
				$htmlresult .= '</div>';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="ln-display-box float-left nc-width-5">';
				$htmlresult .= '&nbsp;';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="ln-display-box float-left nc-width-50 alignlt">';
				$htmlresult .= '<div class="cs-width-400 cs-height-400 noscroll">';
				$htmlresult .= '<img src="'.$tdata["big_image_url"].'" class="auto-wh">';
				$htmlresult .= '</div>';
				$htmlresult .= '</span>';
			...
			if((isset($_GET['edit']) && $_GET['edit'] >= 1) && ($_GET['edit'] == $dataid))
			{
				...
				$htmlresult .= '<h4 class="large blue-font">Updating Gallery</h4><br>';
				$htmlresult .= '<div class="nc-width-40">';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Thumbnail Image Url</small>';
				$htmlresult .= '<input type="url" name="fieldset1" id="fieldset1" placeholder="Enter thumbnail image url" value="'.$tdata["thumbnail_image_url"].'" required="required">';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Big Image Url</small>';
				$htmlresult .= '<input type="url" name="fieldset2" id="fieldset2" placeholder="Enter big image url" value="'.$tdata["big_image_url"].'" required="required">';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Caption</small>';
				$htmlresult .= '<input type="text" name="fieldset3" id="fieldset3" placeholder="Enter caption" value="'.$tdata["detail"].'" required>';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Gallery Type</small>';
				$htmlresult .= '<select name="fieldset4" id="fieldset4" required="required">';
				$htmlresult .= '<option value="'.$tdata["gallery_type"].'" selected="selected">'.$tdata["gallery_type"].'</option>';
				$htmlresult .= '<option value="Resort View">Resort View</option>';
				$htmlresult .= '<option value="Rooms">Rooms</option>';
				$htmlresult .= '<option value="Facilities">Facilities</option>';
				$htmlresult .= '</select>';
				$htmlresult .= '</span>';
				$htmlresult .= '</div>';
				$htmlresult .= '<div class="block-element top-push-10 bottom-push-10 alignrt">';
				$htmlresult .= '<input type="hidden" name="fieldset5" id="fieldset5" value="'.$fieldset.'">';
