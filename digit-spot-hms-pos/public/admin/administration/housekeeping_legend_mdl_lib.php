...
<script>
	...
	function getinputcolor() {
		document.getElementById('fieldset2').value = document.getElementById('colorspicker').value;
	}
...
<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		...
		&nbsp; Note: create new housekeeping legend by clicking <u>new legend</u> button
 	</span>
...
<?php
	...
	{
		createDatabasetable($var_tbl_34); //create a table for this post

		$fieldset1 = escape_data($_POST['fieldset1']);
		$fieldset2 = $_POST['fieldset2'];

		$insert_dataproperty = array("legendname"=>ucwords(strtolower($fieldset1)),"colorcode"=>$fieldset2);
		$insert_constrain = "";
		$data_inserted = mysqli_data_insert($tbL36,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Create new housekeeping legend","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
				...
				<div class="nc-width-40">
					<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
						<div class="bottom-push-20 alignlt">
							<h3 class="nomargin">Creating New Housekeeping Legend</h3>
						</div>
						<span class="block-element bottom-push-10">
							<input type="text" name="fieldset1" id="fieldset1" placeholder="Enter legend name" required="required">
						</span>
						<span class="block-element bottom-push-10">
							<div class="ln-display-box float-left nc-width-80">
								<input type="text" name="fieldset2" id="fieldset2" placeholder="Enter color code" required="required" readonly="readonly">
							</div>
							<div class="ln-display-box float-right nc-width-20">
								<input type="color" id="colorspicker" required="required" onchange="getinputcolor()" class="cs-height-40 anchor">
							</div>
	...
	if((isset($_POST['statusbutton']) && isset($_POST['checkers'])) && (isset($_POST['cstatus']) && !empty($_POST['cstatus'])))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$cstat = mysqli_data_update($tbL36,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Change housekeeping legend status","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['editbutton']))
	{
		...
		$data_inserted = mysqli_data_update($tbL36,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Edit housekeeping legend details","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['deletebutton']) && isset($_POST['checkers']))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$usr_key = array("id"=>$fkey);
			$del = mysqli_data_update($tbL36,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Remove from housekeeping legend list","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if((isset($_POST['searchbutton'])) && (isset($_POST['search']) && !empty($_POST['search']))) {
		$keywords=" AND legendname LIKE '".escape_data($_POST['search'])."%'";
	} else { 
		$keywords="";
	}
	...
	$dataCollect = mysqli_data_fetch($tbL36,$dataproperty,$constrain,$row);

	if(is_array($dataCollect))
	{
		$thproperty = array("noth","legend name","color name","status","noth","enoth");
		...
		foreach($dataCollect as $theader => $tdata)
		{
			...
			$htmlresult .= '<td width="200px" align="center" class="box-border-thick-right">'.$tdata["legendname"].'</td>';
			$htmlresult .= '<td width="150px" align="center" class="box-border-thick-right"><div class="cs-width-20 cs-height-20" style="background:'.$tdata["colorcode"].'">&nbsp;</div></td>';
			...
			if((isset($_GET['edit']) && $_GET['edit'] >= 1) && ($_GET['edit'] == $dataid))
			{
				...
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Legend Name</small>';
				$htmlresult .= '<input type="text" name="fieldset1" id="fieldset1" placeholder="Enter legend name" value="'.$tdata["legendname"].'">';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Color Code</small>';
				$htmlresult .= '<div class="ln-display-box float-left nc-width-80">';
				$htmlresult .= '<input type="text" name="fieldset2" id="fieldset2" value="'.$tdata["colorcode"].'" required="required" readonly="readonly">';
				$htmlresult .= '</div>';
				$htmlresult .= '<div class="ln-display-box float-right nc-width-20">';
				$htmlresult .= '<input type="color" id="colorspicker" required="required" onchange="getinputcolor()" class="cs-height-40 anchor">';
				$htmlresult .= '</div>';
