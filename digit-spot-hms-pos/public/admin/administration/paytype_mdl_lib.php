...
<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		...
		&nbsp; Note: create new pay-type by clicking <u>new paytype</u> button
 	</span>
...
<?php
	...
	{
		createDatabasetable($var_tbl_22); //create a table for this post
		...
		$insert_dataproperty = array("name"=>ucwords(strtolower($fieldset1)),"paytype"=>ucwords(strtolower($fieldset2)),"isreceivable"=>$fieldset3,"iscounter"=>$fieldset4,"isdefault"=>$fieldset5);
		$insert_constrain = array("name"=>ucwords(strtolower($fieldset1)),"isdefault"=>"Yes");
		$data_inserted = mysqli_data_insert($tbL24,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Create new paytype","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
				...
				<div class="nc-width-40">
					<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
						<div class="bottom-push-20 alignlt">
							<h3 class="nomargin">Creating New Paytype</h3>
						</div>
						<span class="block-element bottom-push-10">
							<input type="text" name="fieldset1" id="fieldset1" placeholder="Enter name e.g Cash & Bank Transfer" required="required">
						</span>
						<span class="block-element bottom-push-10">
							<!--<input type="text" name="fieldset2" id="fieldset2" placeholder="Enter paytype" required="required">-->
							<select name="fieldset2" id="fieldset2" required="required">
								<option value="" selected="selected">Choose Pay Type?</option>
								<option value="One-Way Payment">One-Way Payment</option>
								<option value="Two-Way Payment">Two-Way Payment</option>
							</select>
						</span>
						<span class="block-element bottom-push-10">
							<h4 class="large nobold red-font alignlt bottom-pull-5">* Always choose default YES for cash mode only</h4>
							<select name="fieldset5" id="fieldset5" required="required">
								<option value="" selected="selected">Is Default?</option>
								<option value="Yes">Yes, only for cash mode</option>
								<option value="No">No</option>
							</select>
						</span>
						<span class="block-element bottom-push-10">
							<select name="fieldset3" id="fieldset3" required="required">
								<option value="" selected="selected">Is GC receivable?</option>
								<option value="Yes">Yes</option>
								<option value="No">No</option>
							</select>
						</span>
						<span class="block-element bottom-push-10">
							<h4 class="large nobold red-font alignlt bottom-pull-5">* Choose NO for coupon mode</h4>
							<select name="fieldset4" id="fieldset4" required="required">
								<option value="" selected="selected">Is Counter Use?</option>
								<option value="Yes">Yes</option>
								<option value="No">No</option>
							</select>
						</span>
						...
						<br><h4 class="large nobold dark-grey-font">For two-way payment, follow proper naming convention e.g Cash & Bank Transfer</h4>
						<br><br>
	...
	if(isset($_POST['editbutton']))
	{
		...
		$insert_dataproperty = array("name"=>ucwords(strtolower($fieldset1)),"paytype"=>ucwords(strtolower($fieldset2)),"isreceivable"=>$fieldset3,"iscounter"=>$fieldset5);
		$insert_constrain = array("id"=>$fieldset4);
		$data_inserted = mysqli_data_update($tbL24,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Edit paytype details","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['deletebutton']) && isset($_POST['checkers']))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$del = mysqli_data_update($tbL24,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Remove from paytype list","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if((isset($_POST['searchbutton'])) && (isset($_POST['search']) && !empty($_POST['search']))) {
		$keywords=" AND (name LIKE '".escape_data($_POST['search'])."%' OR paytype LIKE '".escape_data($_POST['search'])."%')";
	} else { 
		$keywords="";
	}
	...
	$dataproperty = "id,name,paytype,isreceivable,iscounter,isdefault";
	$constrain = array("deletedata"=>0);
	$row = "array";

	$dataCollect = mysqli_data_fetch($tbL24,$dataproperty,$constrain,$row);

	if(is_array($dataCollect))
	{
		$thproperty = array("noth","name","pay type","is counter use","is receivable","noth","enoth");
		...
		$num=$pgstart; $g=""; $dataid=""; $mydefault="";
		...
		foreach($dataCollect as $theader => $tdata)
		{
			...
			if($tdata['isdefault'] == 'Yes') { $mydefault = "* "; }
			else { $mydefault = ""; }
					
			$htmlresult .= '<tr bgcolor="'.$trcolor.'">';
			$htmlresult .= '<td width="70px" class="box-border-thick-right" align="center">'.$num.'</td>';
			$htmlresult .= '<td width="200px" align="center" class="box-border-thick-right">'.$mydefault.$tdata["name"].'</td>';
			$htmlresult .= '<td width="200px" align="center" class="box-border-thick-right">'.$tdata["paytype"].'</td>';
			$htmlresult .= '<td width="200px" align="center" class="box-border-thick-right">'.$tdata["iscounter"].'</td>';
			$htmlresult .= '<td width="150px" align="center" class="box-border-thick-right">'.$tdata["isreceivable"].'</td>';
			...
			if((isset($_GET['edit']) && $_GET['edit'] >= 1) && ($_GET['edit'] == $dataid))
			{
				...
				$htmlresult .= '<input type="text" name="fieldset1" id="fieldset1" placeholder="Enter name" value="'.$tdata["name"].'">';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Pay Type</small>';
				//$htmlresult .= '<input type="text" name="fieldset2" id="fieldset2" placeholder="Enter paytype" value="'.$tdata["paytype"].'">';
				$htmlresult .= '<select name="fieldset2" id="fieldset2" required="required"><option value="'.$tdata["paytype"].'" selected="selected">'.$tdata["paytype"].'</option><option value="One-Way Payment">One-Way Payment</option>
								<option value="Two-Way Payment">Two-Way Payment</option></select>';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">IsCounter Use</small>';
				$htmlresult .= '<select name="fieldset5" id="fieldset5" required="required">';
				$htmlresult .= '<option value="'.$tdata["iscounter"].'" selected="selected">'.$tdata["iscounter"].'</option>';
				$htmlresult .= '<option value="Yes">Yes</option>';
				$htmlresult .= '<option value="No">No</option>';
				$htmlresult .= '</select>';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">IsReceivable</small>';
				$htmlresult .= '<select name="fieldset3" id="fieldset3" required="required">';
				$htmlresult .= '<option value="'.$tdata["isreceivable"].'" selected="selected">'.$tdata["isreceivable"].'</option>';
				$htmlresult .= '<option value="Yes">Yes</option>';
				$htmlresult .= '<option value="No">No</option>';
				$htmlresult .= '</select>';
				$htmlresult .= '</span>';
