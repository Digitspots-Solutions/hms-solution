...
<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		...
		&nbsp; Note: create new room status legend by clicking <u>new legend</u> button
 	</span>
...
<?php
	...
	{
		createDatabasetable($var_tbl_36); //create a table for this post
		...
		$insert_constrain = "";
		$data_inserted = mysqli_data_insert($tbL38,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Create new room status legend","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
				...
				<div class="nc-width-40">
					<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
						<div class="bottom-push-20 alignlt">
							<h3 class="nomargin">Creating New Room Status Legend</h3>
						</div>
	...
	if((isset($_POST['statusbutton']) && isset($_POST['checkers'])) && (isset($_POST['cstatus']) && !empty($_POST['cstatus'])))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$cstat = mysqli_data_update($tbL38,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Change room status legend status","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['editbutton']))
	{
		...
		$data_inserted = mysqli_data_update($tbL38,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Edit room status legend details","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['deletebutton']) && isset($_POST['checkers']))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$usr_key = array("id"=>$fkey);
			$del = mysqli_data_update($tbL38,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Remove from room status legend list","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
