...
<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		...
		&nbsp; Note: create new counter by clicking <u>new counter</u> button
 	</span>
...
<?php
	...
	{
		createDatabasetable($var_tbl_17); //create a table for this post

		$fieldset1 = escape_data($_POST['fieldset1']);
		$fieldset2 = escape_data($_POST['fieldset2']);
		$fieldset3 = escape_data($_POST['fieldset3']);

		$insert_dataproperty = array("countername"=>ucwords(strtolower($fieldset1)),"countertype"=>$fieldset2,"detail"=>$fieldset3);
		$insert_constrain = "";
		$data_inserted = mysqli_data_insert($tbL19,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Create new counter","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
				...
				<div class="nc-width-40">
					<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
						<div class="bottom-push-20 alignlt">
							<h3 class="nomargin">Creating New Counter</h3>
						</div>
						<span class="block-element bottom-push-10">
							<input type="text" name="fieldset1" id="fieldset1" placeholder="Enter counter name" required="required">
						</span>
						<span class="block-element bottom-push-10">
							<select name="fieldset2" id="fieldset2" required="required">
								<option value="" selected="selected">Choose Type</option>
								<option value="frontdesk counter">Frontdesk Counter</option>
	...
	if(isset($_POST['editbutton']))
	{
		...
		$data_inserted = mysqli_data_update($tbL19,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Edit counter details","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['deletebutton']) && isset($_POST['checkers']))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$del = mysqli_data_update($tbL19,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Remove from counter list","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if((isset($_POST['searchbutton'])) && (isset($_POST['search']) && !empty($_POST['search']))) {
		$keywords=" AND countername LIKE '".escape_data($_POST['search'])."%'";
	} else { 
		$keywords="";
	}
	...
	$dataproperty = "id,countername,countertype,detail,status";
	$constrain = array("deletedata"=>0);
	$row = "array";

	$dataCollect = mysqli_data_fetch($tbL19,$dataproperty,$constrain,$row);

	if(is_array($dataCollect))
	{
		$thproperty = array("noth","counter name","counter type","description","status","noth","enoth");
		...
		$htmlresult .= '<input type="text" name="search" id="search" placeholder="Search by counter name.." onkeyup="chgclass('.$fxobj.','.$fxclass.')">';
		$htmlresult .= '</div>';
		...
		foreach($dataCollect as $theader => $tdata)
		{
			...
			$htmlresult .= '<td width="200px" align="center" class="box-border-thick-right">'.$tdata["countername"].'</td>';
			$htmlresult .= '<td width="200px" align="center" class="box-border-thick-right">'.ucwords($tdata["countertype"]).'</td>';
			...
			if((isset($_GET['edit']) && $_GET['edit'] >= 1) && ($_GET['edit'] == $dataid))
			{
				...
				$htmlresult .= '<h4 class="large blue-font">Updating '.$tdata["countername"].'</h4><br>';
				$htmlresult .= '<div class="nc-width-40">';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Counter Name</small>';
				$htmlresult .= '<input type="text" name="fieldset1" id="fieldset1" placeholder="Enter counter name" value="'.$tdata["countername"].'">';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Counter Type</small>';
				$htmlresult .= '<select name="fieldset2" id="fieldset2" required="required">';
				$htmlresult .= '<option value="'.$tdata["countertype"].'" selected="selected">'.ucwords($tdata["countertype"]).'</option>';
				$htmlresult .= '<option value="frontdesk counter">Frontdesk Counter</option>';
				$htmlresult .= '<option value="pos counter">Pos Counter</option>';
				$htmlresult .= '</select>';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
