...
<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		...
		&nbsp; Note: create company tax charges by clicking <u>new tax</u> button
 	</span>
...
<?php
	...
	$get_tax_list = arrayset_form($tax_charges,'select');

	#-----------------------------------------------------------------------------------------------------------------

	if(isset($_POST['submitbutton']))
	{
		createDatabasetable($var_tbl_33); //create a table for this post
		
		$fieldset1 = escape_data($_POST['fieldset1']);
		$fieldset2 = 0; //escape_data($_POST['fieldset2']);
		$fieldset3 = escape_data($_POST['fieldset3']);
		$fieldset4 = escape_data($_POST['fieldset4']);
		$fieldset5 = escape_data($_POST['fieldset5']);

		$insert_dataproperty = array("taxname"=>$fieldset1,"taxcharge"=>$fieldset2,"detail"=>$fieldset3,"isactive"=>$fieldset4,"isonlinebooking"=>$fieldset5);
		$insert_constrain = "";
		$data_inserted = mysqli_data_insert($tbL35,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Create new hotel tax charge","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
				...
				<div class="nc-width-40">
					<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
						<div class="bottom-push-20 alignlt">
							<h3 class="nomargin">Creating New Tax Charge</h3>
						</div>
						<span class="block-element bottom-push-10">
							<!--<input type="text" name="fieldset1" id="fieldset1" placeholder="Enter tax name" required="required">-->
							<select name="fieldset1" id="fieldset1" required="required">
								<option value="" selected="selected">Select Tax Name</option>
								<?php echo $get_tax_list; ?>
							</select>
						</span>
						<!--
						<span class="block-element bottom-push-10">
							<div class="ln-display-box float-left nc-width-90">
								<input type="number" name="fieldset2" id="fieldset2" step="any" placeholder="Enter charges" required="required">
							</div>
						...
						<span class="block-element bottom-push-10">
							<select name="fieldset5" id="fieldset5" required="required">
								<option value="" selected="selected">Is Booking Online Applicable</option>
								<option value="Yes">Yes</option>
								<option value="No">No</option>
							</select>
						</span>
	...
	if(isset($_POST['editbutton']))
	{
		...
		$data_inserted = mysqli_data_update($tbL35,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Edit hotel tax details","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['deletebutton']) && isset($_POST['checkers']))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$del = trash_record($tbL35,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Remove hotel tax list","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if((isset($_POST['searchbutton'])) && (isset($_POST['search']) && !empty($_POST['search']))) {
		$keywords=" AND detail REGEXP '".escape_data($_POST['search'])."'";
	} else { 
		$keywords="";
	}
	...
	$dataproperty = "id,taxname,taxcharge,detail,isactive,isonlinebooking";
	$constrain = array("deletedata"=>0);
	$row = "array";

	$dataCollect = mysqli_data_fetch($tbL35,$dataproperty,$constrain,$row);

	if(is_array($dataCollect))
	{
		$thproperty = array("noth","tax name","description","isactive","isonline booking applicable","noth","enoth");
		...
		$num=$pgstart; $g=""; $dataid=""; $this_charge_name="";
		...
		foreach($dataCollect as $theader => $tdata)
		{
			...
			$dataid = $tdata["id"];
			$this_charge_name = arrayget_key($tax_charges,$tdata["taxname"]);
			...
			$htmlresult .= '<td width="200px" align="center" class="box-border-thick-right">'.$this_charge_name.'</td>';
			//$htmlresult .= '<td width="100px" align="center" class="box-border-thick-right">0</td>';
			...
			$htmlresult .= '<td width="150px" align="center" class="box-border-thick-right blue-font">'.$tdata["isonlinebooking"].'</td>';
			...
			if((isset($_GET['edit']) && $_GET['edit'] >= 1) && ($_GET['edit'] == $dataid))
			{
				...
				$htmlresult .= '<h4 class="large blue-font">Updating '.$tdata["taxname"].'</h4><br>';
				$htmlresult .= '<div class="nc-width-40">';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Tax Name</small>';
				//$htmlresult .= '<input type="text" name="fieldset1" id="fieldset1" placeholder="Enter tax name" value="'.$tdata["taxname"].'" required="required">';
				$htmlresult .= '<select name="fieldset1" id="fieldset1" required="required">';
				$htmlresult .= '<option value="'.$tdata["taxname"].'" selected="selected">'.$this_charge_name.'</option>';
				$htmlresult .= $get_tax_list;
				$htmlresult .= '</select>';
				$htmlresult .= '</span>';
				//$htmlresult .= '<span class="block-element bottom-push-10">';
				//$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Charge (%)</small>';
				//$htmlresult .= '<input type="number" name="fieldset2" id="fieldset2" step="any" placeholder="Enter discount" value="'.$tdata["taxcharge"].'" required="required">';
				//$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Description</small>';
				$htmlresult .= '<textarea name="fieldset3" id="fieldset3" placeholder="Enter description" required="required">'.$tdata["detail"].'</textarea>';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				...
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">IsOnline Booking Applicable</small>';
				$htmlresult .= '<select name="fieldset5" id="fieldset5" required="required">';
				$htmlresult .= '<option value="'.$tdata["isonlinebooking"].'" selected="selected">'.$tdata["isonlinebooking"].'</option>';
				$htmlresult .= '<option value="Yes">Yes</option>';
				$htmlresult .= '<option value="No">No</option>';
				$htmlresult .= '</select>';
				$htmlresult .= '</span>';
	...
	$additionalQuery = "";
	mysqli_data_check($tbL35,'(*)',$constrain);
