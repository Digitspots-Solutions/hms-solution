...
<?php
	...
	if(isset($_POST['submitbutton']))
	{
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Update company details","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
...
<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
	<div class="block-element box-border-thick pads20 sml-rounded-button bottom-push-20">
		...
		<span class="block-element nc-width-100 bottom-push-30">
			...
			<textarea name="fieldset18" id="fieldset18" placeholder="e.g Westland Stores" required="required"><?php if(isset($get_hotel_data[17])) { echo $get_hotel_data[17]; } ?></textarea>
		</span>
