...
<div class="pads30" id="section-to-print">

	<div class="cs-width-100 bottom-push-10 noscroll">
		<img src="<?php echo _FC_LOGO_Mx; ?>" class="auto-wh">
	</div>
	<h4 class="large nobold">Current Time: <b class="nobold default-text-font-bold"><?php echo date('d/m/Y',strtotime($server_get_date)).' '.$server_get_time; ?></b></h4>
	<h4 class="large nobold">Audit Done Till: <b class="nobold default-text-font-bold"><?php echo date('d/m/Y',strtotime($audit_done_date)); ?></b></h4>
	...
	<?php
		...
		$inhouse_guest_dataset = "SELECT * FROM guest_occupancy_detail_tbl WHERE status IN('CheckedIn','CheckedOut') AND checkin_date='{$bill_post_date}'";
	...
	<table cellpadding="3" cellspacing="0">
		...
		<?php
			...
			if($inhouse_guest_rows == true) {
				...
				$roomtype = ""; $room = ""; $salutation = ""; $roomlist = ""; $booking_type = "";
				...
				while($data = @mysqli_fetch_array($inhouse_guest_query,MYSQLI_ASSOC)) {
					...
					$select_fx = "roomid='{$data['roomid']}' AND charge='Yes' AND ischarged=1";
					...
					$room_tariff = $amt_sx['rmAmt']; $discount = $amt_sx['rmAmt'];
					$tax_charge = $amt_fx['taxAmt'] + $amt_fx['scAmt'] + $amt_fx['ctaxAmt'];

					$select_tx = "roomid='{$data['roomid']}' AND payment='Pending'";
					...
					?>
						<tr>
							...
							<td align="center" class="<?php echo $color_code; ?>"><?php echo $guest_name; ?></td>
	...
	<?php
		...
		$noshow_guest_dataset = "SELECT * FROM guest_occupancy_detail_tbl WHERE status='No Show' AND checkin_date='{$bill_post_date}'";
	...
	<?php
		...
		$ccSql = "SELECT SUM(cancellation_charges) AS 'ccAmt' FROM guest_occupancy_detail_tbl WHERE cancel_date='{$bill_post_date}'"; $query_cc = mysqli_query($mysqli,$ccSql);
	...
	<?php
		...
		$ctgSql = "SELECT categoryid FROM pos_store_product_tbl WHERE deletedata=0 GROUP BY categoryid";
			...
			<?php
				if($row_ctg == true) {
					...
					while($ctg = @mysqli_fetch_array($query_ctg,MYSQLI_ASSOC)) {
						...
						$ctgval = "SELECT id FROM pos_store_product_tbl WHERE deletedata=0 AND categoryid={$ctg['categoryid']}";
						$query_ctgval = mysqli_query($mysqli,$ctgval);

						$getSum = 0; $sumfx = "";

						while($order = @mysqli_fetch_array($query_ctgval,MYSQLI_ASSOC)) {
							$sumfx = "SELECT SUM(price * qty) AS 'tsales' FROM pos_orders_tbl WHERE itemid={$order['id']} AND status='Completed' AND datelogged='{$bill_post_date}' AND isreversed=0";
...
<p class="pads20 alignct">
	...
	<a href="<?php echo DOMAIN_URL.PUB_FLD; ?>admin/portal" class="dark-blue-font">Return to workspace</a>
</p>
