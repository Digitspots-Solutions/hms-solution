...
if(isset($_GET['orderno'])) {
	$keywords = " AND order_number='{$_GET['orderno']}'";
	$disableHeaders = 0;
	unset($_GET['orderno']);
} else {
...
$lAr = "SELECT * FROM {$tbl} WHERE store_type IN('Virtual Stores') AND deletedata=0 GROUP BY order_number";
...
<script src="materialcontrol/js/jspath.js"></script>
<script src="materialcontrol/js/jsfx.js"></script>
<script src="materialcontrol/js/index.js"></script>
			...
			if($disableHeaders == 0) {
				?>
					<div class="float-right cs-width-250 box-border-thick sml-rounded-button pads15 alignlt">
						<h4 class="xlarge nobold default-text-font-bold">All Orders</h4>
					...
					<div class="alignlt"><h3 class="xlarge nobold nomargin"><a href="<?php echo $ths_page; ?>" title="Refresh" class="right-push-10"><img src="<?php echo DOMAIN_URL; ?>theme/images/general/refresh.png"></a> Check for order status here to continue managing the PR.</h3></div>
			...
			if($po['isdata'] == true) {
						...
						?>
							<div class="box-border-thick xsml-rounded-button pads20 bottom-push-20">
								<span class="float-left">
									...
									<h3 class="xlarge nobold nunito-semibold"><?php echo $_gparams[$var_supplier]['returnval']; ?><b class="fa-arrow-right left-push-10"></b></h3>
								</span>
				...
				?>
					...
					<div class="block-element" align="center">
						<h3 class="large nobold dark-grey-font">No default records list. Use search form</h3>
					</div>
				<?php
			}
		?>
...
<script>
	...
	function getrqi(obj) {
		var isuri = window.location.href;
		
		if(isuri.indexOf('?orderno') > -1) {
			splituri = isuri.split('?');
			isuri = splituri[0];
		} else {
			isuri = window.location.href;
		}

		window.location.href = isuri+"?orderno="+obj.value;
	}
