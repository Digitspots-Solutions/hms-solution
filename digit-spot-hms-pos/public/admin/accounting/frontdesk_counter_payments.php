...
$get_users = select_dt_fetch('account_src','frontdesk',$tbL158,'','userid');
...
if(isset($_POST['applybutton']) && isset($_POST['wgid'])) {
	...
	if((isset($_POST['charge4diff']) && !empty($_POST['charge4diff'])) && ($diff_amount > 0)) {
		...
		$cityledger_sql = array("forcounter"=>"frontdesk","amount"=>$diff_amount,"chargeto"=>$chargeto,"detail"=>$message,"userid"=>$userSignedIn,"datelogged"=>$server_get_date,"timelogged"=>$server_get_time);
		...
		<div class="cs-width-1500">
			<div id="section-to-print">
				<?php
					...
					$queryset = "deletedata=0 AND account_src='frontdesk'".$keywords;
					...
					$keys = array(
						...
						"shift"=>"shift",
						"userid"=>"cashier",
						"paid_amount"=>"(nf)amount receivable &#8358;",
						"actual_amount"=>"(nf)amount received &#8358;",
						"diff_amount"=>"(nf)amount diff. &#8358;",
						"remark"=>"remark",
						"status"=>"status",
				...
				<div id="rBox" class="fx-width-40 pads30 white-theme obj-shadow xsml-rounded-button alignlt">
					<form action="" method="post" autocomplete="off">
						<div class="pads10 alignlt">
							...
							<input type="text" name="wgtamountreceived" id="wgtamountreceived" placeholder="Enter amount here" onkeyup="numberinputFormat(this.value,this.id,'amountreceived')" class="no-back-black default-text-font-bold">
...
<script>
		...
		if(fr == 'reportform') {
			...
			setTimeout(() => {
				if(sessionStorage.getItem('acctgparams') !== null && sessionStorage.getItem('acctgparams') != 'undefined') {
					var uri,params,wp;
					uri = sessionStorage.getItem('accturi'); params = sessionStorage.getItem('acctgparams'); wp = JSON.parse(params);
	...
	window.onload = () => {
		if(sessionStorage.getItem('accturi') == null || sessionStorage.getItem('accturi') == 'undefined') {
			sessionStorage.setItem('accturi',window.location.href);
		}
