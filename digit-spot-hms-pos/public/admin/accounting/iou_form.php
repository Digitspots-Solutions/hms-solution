...
$sqlJo = "SELECT order_number FROM {$tbL121} WHERE pr_status IN('Job Order') AND receipt_status IN('Pending') AND deletedata=0 GROUP BY order_number ORDER BY id DESC"; $showJo = wgetSQL($sqlJo);
						...
						<ul class="nolist">
							...
							<li class="ln-display-box float-left box-border-thick-bottom right-pull-20 left-pull-20 bottom-push-3 right-push-15">
								...
								<div id="forjoborder" class="xform noshow">
									<select name="joborders" id="joborders" class="nopads no-back-black" onchange="getjoamt(this.value); getjo(this.value)">
...
<script>
	...
	function getjoamt(val) {

		sqldatastring.sql = "SELECT SUM(order_net_amount) AS totalamount FROM stock_item_purchase_order_tbl WHERE pr_status IN('Job Order') AND order_number='"+val+"'";
		...
		function wgtpop(response) {
			...
			htmlpassval(numberFormat(data[0].totalamount),'wgtamount');
			htmlpassval(data[0].totalamount,'amount');
		}
	}
