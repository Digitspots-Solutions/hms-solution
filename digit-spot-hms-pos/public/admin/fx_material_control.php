...
$row = "array";
$constrain = array("moduleid"=>7,"status"=>"Active");
...
if(isset($myaccess) && $myaccess == 'super admin') {
	...
	if(is_array($dataCollect))
	{
							...
							<div id="xlmdl1<?php echo $cvalue['id']; ?>" class="noshow motion">
								<?php
									foreach ($submdl_data as $subkey => $subvalue) {
										?>
											<div class="block-element top-pull-10 right-pull-10 bottom-pull-10 left-pull-10 anchor box-border-thick-bottom  sub-menu-class motion" onclick="mx_loadframe('<?php echo $subvalue['name']; ?>','<?php echo $subvalue['id']; ?>')">
		...
		foreach ($dataCollect as $xckey => $xcvalue) {
			...
			if($submdls == 0) {
				...
				?>
				<div id="mdl1<?php echo $cvalue['id']; ?>" class="block-element">
					<div class="block-element top-pull-10 right-pull-10 bottom-pull-10 left-pull-10 anchor box-border-thick-bottom motion nosub-menu-class" onclick="mx_loadframe('<?php echo $xcvalue['category']; ?>','<?php echo $xcvalue['id']; ?>')">
	...
	?>
		<input type="hidden" id="mtc-mode" value="1">
	<?php
}
else
{
	$isprivilege = perform_role_check($tbL5,$myrole,7,0);
