...
<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		...
		&nbsp; Note: create your inclusions for packages by clicking <u>new inclusion</u> button
 	</span>
...
<?php
	...
	$posstore = select_dt_fetch('status','Active',$tbL14,'id','posname');

	#-----------------------------------------------------------------------------------------------------------------

	if(isset($_POST['submitbutton']))
	{
		createDatabasetable($var_tbl_79); //create a table for this post
		...
		$insert_dataproperty = array("name"=>ucwords(strtolower($fieldset1)),"detail"=>$fieldset2,"price"=>$fieldset3,"posstore"=>$fieldset4,"posproduct"=>$fieldset5,"status"=>$fieldset6);
		$insert_constrain = "";
		$data_inserted = mysqli_data_insert($tbL83,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Create new inclusion","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
				...
				<div class="nc-width-40">
					<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
						<div class="bottom-push-20 alignlt">
							<h3 class="nomargin">Creating New Inclusion</h3>
						</div>
						...
						<span class="block-element bottom-push-10">
							<input type="text" name="fieldset3" id="fieldset3" placeholder="Enter price" pattern="\d*" maxlength="5" required="required">
						</span>
						<span class="block-element bottom-push-15">
							<select name="fieldset4" id="fieldset4" onchange="">
								<option value="" selected="selected">Pos Store</option>
								<?php echo $posstore; ?>
							</select>
						</span>
						<span class="block-element bottom-push-15">
							<select name="fieldset5" id="fieldset5">
								<option value="" selected="selected">Pos Store Product</option>
							</select>
						</span>
						<span class="block-element bottom-push-10">
	...
	if((isset($_POST['statusbutton']) && isset($_POST['checkers'])) && (isset($_POST['cstatus']) && !empty($_POST['cstatus'])))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$cstat = mysqli_data_update($tbL83,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Change inclusion status","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['editbutton']))
	{
		...
		$insert_dataproperty = array("name"=>ucwords(strtolower($fieldset1)),"detail"=>$fieldset2,"price"=>$fieldset3,"posstore"=>$fieldset4,"posproduct"=>$fieldset5);
		$insert_constrain = array("id"=>$fieldset6);
		$data_inserted = mysqli_data_update($tbL83,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Edit inclusion details","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['deletebutton']) && isset($_POST['checkers']))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$usr_key = array("id"=>$fkey);
			$del = mysqli_data_update($tbL83,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Remove from inclusion list","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	$dataproperty = "id,name,detail,price,posstore,posproduct,status";
	$constrain = array("deletedata"=>0);
	$row = "array";

	$dataCollect = mysqli_data_fetch($tbL83,$dataproperty,$constrain,$row);

	if(is_array($dataCollect))
	{
		$thproperty = array("noth","name","description","price","pos store","pos store product","status","noth","enoth");
		...
		foreach($dataCollect as $theader => $tdata)
		{
			...
			if($tdata["posstore"] >= 1) {
				$getposstore = idget_data($tbL14,$tdata["posstore"],'posname');
			} else {
				$getposstore = 'Not available';
			}

			if($tdata["posproduct"] >= 1) {
				$getposstore_product = '';
			} else {
				$getposstore_product = 'Not available';
			}
			...
			$htmlresult .= '<td width="100px" align="center" class="box-border-thick-right">&#8358 '.number_format($tdata["price"],2).'</td>';
			$htmlresult .= '<td width="150px" align="center" class="box-border-thick-right">'.$getposstore.'</td>';
			$htmlresult .= '<td width="150px" align="center" class="box-border-thick-right">'.$getposstore_product.'</td>';
			...
			if((isset($_GET['edit']) && $_GET['edit'] >= 1) && ($_GET['edit'] == $dataid))
			{
				...
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Salutation</small>';
				...
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Price</small>';
				$htmlresult .= '<input type="text" name="fieldset3" id="fieldset3" placeholder="Enter price" pattern="\d*" maxlength="5" required="required" value="'.$tdata["price"].'">';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Pos Store</small>';
				$htmlresult .= '<select name="fieldset4" id="fieldset4" required="required">';
				$htmlresult .= '<option value="'.$tdata["posstore"].'" selected="selected">'.$getposstore.'</option>';
				$htmlresult .= $posstore;
				$htmlresult .= '</select>';
				$htmlresult .= '</span>';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Pos Store Product</small>';
				$htmlresult .= '<select name="fieldset5" id="fieldset5" required="required">';
				$htmlresult .= '<option value="'.$tdata["posproduct"].'" selected="selected">'.$getposstore_product.'</option>';
				$htmlresult .= '</select>';
				$htmlresult .= '</span>';
				$htmlresult .= '</div>';
	...
	$additionalQuery = "";
	mysqli_data_check($tbL83,'(*)',$constrain);
