...
<?php
	...
	if(isset($_POST['submitbutton']))
	{
		...
		for($a=0; $a < count($room_types); $a++)
		{
			...
			if($corp_tariff_status[$a] == 'Active') {
				
				$post_tax_name = "tax".$corporateid;
				
				if(isset($_POST[$post_tax_name]) && !empty($_POST[$post_tax_name])) {
					foreach($_POST[$post_tax_name] as $tax_incl) {
						$tax_incl_query = array("room_type_id"=>$room_types[$a],"corporateid"=>$corporateid,"taxid"=>$tax_incl);
						mysqli_data_insert($tbL82,$tax_incl_query,$tax_incl_query);
						$tax_incl_query = "";
					}
				}

			} elseif($corp_tariff_status[$a] == 'InActive') {
				...
				if(isset($_POST[$post_tax_name]) && !empty($_POST[$post_tax_name])) {
					foreach($_POST[$post_tax_name] as $tax_incl) {
						$tax_incl_query = array("room_type_id"=>$room_types[$a],"corporateid"=>$corporateid,"taxid"=>$tax_incl);
						trash_record($tbL82,$tax_incl_query);
						$tax_incl_query = "";
					}
				}
			}
									...
									foreach ($room_types as $rm_key => $rm_value) {
										...
										?>
											<tr>
															...
															<td width="200px" class="box-border-thick-right">
																<?php
																	...
																	foreach ($select_tax as $tkey => $tvalue) {
																		...
																		?>
																			<div class="block-element bottom-push-3"><input type="checkbox" name="tax<?php echo $crp; ?>[]" value="<?php echo $tvalue['taxname']; ?>"<?php if(isset($istax_inclusive[0]) && $istax_inclusive[0] >= 1) { ?> checked<?php } ?>> <?php echo $th_taxname; ?></div>
																		<?php
																	}
																?>
															</td>
												...
												?>
												<td width="100px" class="box-border-thick-right"><input type="number" step="any" name="mon_rates[]" id="mon<?php echo $count_r; ?>"<?php if(isset($mon_adultrate_data[0]) && !empty($mon_adultrate_data[0])) { ?> value="<?php echo $mon_adultrate_data[0]; ?>"<?php } else { ?> value="<?php echo $rm_value['defaultprice']; ?>"<?php } ?>><input type="hidden" name="mondays[]" value="monday"></td>
												<td width="100px" class="box-border-thick-right"><input type="number" step="any" name="tue_rates[]" id="tue<?php echo $count_r; ?>"<?php if(isset($tue_adultrate_data[0]) && !empty($tue_adultrate_data[0])) { ?> value="<?php echo $tue_adultrate_data[0]; ?>"<?php } else { ?> value="<?php echo $rm_value['defaultprice']; ?>"<?php } ?>><input type="hidden" name="tuesdays[]" value="tuesday"></td>
												<td width="100px" class="box-border-thick-right"><input type="number" step="any" name="wed_rates[]" id="wed<?php echo $count_r; ?>"<?php if(isset($wed_adultrate_data[0]) && !empty($wed_adultrate_data[0])) { ?> value="<?php echo $wed_adultrate_data[0]; ?>"<?php } else { ?> value="<?php echo $rm_value['defaultprice']; ?>"<?php } ?>><input type="hidden" name="wedsdays[]" value="wednesday"></td>
												<td width="100px" class="box-border-thick-right"><input type="number" step="any" name="thur_rates[]" id="thur<?php echo $count_r; ?>"<?php if(isset($thur_adultrate_data[0]) && !empty($thur_adultrate_data[0])) { ?> value="<?php echo $thur_adultrate_data[0]; ?>"<?php } else { ?> value="<?php echo $rm_value['defaultprice']; ?>"<?php } ?>><input type="hidden" name="thursdays[]" value="thursday"></td>
												<td width="100px" class="box-border-thick-right"><input type="number" step="any" name="fri_rates[]" id="fri<?php echo $count_r; ?>"<?php if(isset($fri_adultrate_data[0]) && !empty($fri_adultrate_data[0])) { ?> value="<?php echo $fri_adultrate_data[0]; ?>"<?php } else { ?> value="<?php echo $rm_value['defaultprice']; ?>"<?php } ?>><input type="hidden" name="fridays[]" value="friday"></td>
												<td width="100px" class="box-border-thick-right"><input type="number" step="any" name="sat_rates[]" id="sat<?php echo $count_r; ?>"<?php if(isset($sat_adultrate_data[0]) && !empty($sat_adultrate_data[0])) { ?> value="<?php echo $sat_adultrate_data[0]; ?>"<?php } else { ?> value="<?php echo $rm_value['defaultprice']; ?>"<?php } ?>><input type="hidden" name="saturdays[]" value="saturday"></td>
												<td width="100px" class="box-border-thick-right"><input type="number" step="any" name="sun_rates[]" id="sun<?php echo $count_r; ?>"<?php if(isset($sun_adultrate_data[0]) && !empty($sun_adultrate_data[0])) { ?> value="<?php echo $sun_adultrate_data[0]; ?>"<?php } else { ?> value="<?php echo $rm_value['defaultprice']; ?>"<?php } ?>><input type="hidden" name="sundays[]" value="sunday"></td>
											</tr>
										<?php
									}
								}
