<?php $smdl = "recreation"; $logs = escape_data($_GET['logs']); $salutations = select_dt_fetch('status','Active',$tbL42,'id','name'); $duration = arrayset_form($recreation_duration,'select'); $list_payment_modes = select_dt_fetch('',0,$tbL24,'id','name');
...
<?php
	...
	if(isset($_POST['submitbutton']))
	{
		...
		if(isset($isdata) && $isdata == 2) {
			...
			?>
				<script>
					alert("Notification\n\nRecreation membership was done successfully. Go to recreation dashboard to see the new registration");
				</script>
