...
if(isset($_POST['uri']) && $_POST['uri'] == 'apply-fund-to-float') {
	
	$sales_counter_query = array("counterid"=>$counter_sesid,"userid"=>$userSignedIn,"fundid"=>1,"ispast"=>0);
	$sales_counter_data = mysqli_data_fetch($tbL25,'openingbalance',$sales_counter_query,'noarray');

	$new_collection = $sales_counter_data[0] + $_POST['float'];
			
	$sales_counter_sql = array("openingbalance"=>$new_collection);
	...
	$post_header = "Notification";
	$post_message = "Fund added successfully";
	
	$islogfile = 1;
	$logfile_msg = "Fund added {$_POST['float']} to opening balance by this user";
}

#-------------------------------------------------------------------------------------------------------------------

if(isset($_POST['uri']) && $_POST['uri'] == 'apply-withdraw-fund') {
	
	$sales_counter_query = array("counterid"=>$counter_sesid,"userid"=>$userSignedIn,"fundid"=>1,"ispast"=>0);
	$sales_counter_data = mysqli_data_fetch($tbL25,'openingbalance,withdrawal',$sales_counter_query,'noarray');

	$withdraw_bal = $sales_counter_data[0] - $sales_counter_data[1];
	
	if($withdraw_bal >= $_POST['float']) {
		
		$new_collection = $sales_counter_data[1] + $_POST['float'];
				
		$sales_counter_sql = array("withdrawal"=>$new_collection);
		mysqli_data_update($tbL25,$sales_counter_sql,$sales_counter_query);
		...
		$post_header = "Notification";
		$post_message = "Fund withdrawn successfully";
		
		$islogfile = 1;
		$logfile_msg = "Fund withdrawn {$_POST['float']} from the opening balance amount by this user";
		...
		$post_header = "Notification";
		$post_message = "Insufficient fund to carry out the request";
...
$wgt_gc = ($wgt_crd + $wgt_obl) - ($wgt_rf + $wgt_wdr);
...
$pst_field = "shiftid,resumptiontime,closetime,datelogged,dateclosed,status";
...
if(isset($_POST['uri']) && $_POST['uri'] == 'apply-transfer-to-gc') {
	...
	$sqlsetx = "SUM(bill_amount)";
	$querysetx = "shiftid={$current_shift} AND cashier={$cashier} AND media NOT IN(0) AND datelogged='{$server_get_date}' AND isreversed=0 AND deletedata=0"; $wgt_gco_funds = mysqli_arithmetic_data($tbL100,$sqlsetx,$querysetx);
	if($wgt_gco_funds >= 1) { $amount = $amount - $wgt_gco_funds; } else { $amount = $amount; }

	$pst_query = "";
	$pst_field = array("account"=>$acct_src,"shift_start_date"=>$start_end_date,"shift_end_date"=>$server_get_date,"shift_start_time"=>$start_time,"shift_end_time"=>$server_get_time,"shift"=>$shift,"counterid"=>$counter,"shiftid"=>$current_shift,"userid"=>$cashier,"paid_amount"=>$amount,"account_src"=>"frontdesk","datelogged"=>$server_get_date,"timelogged"=>$server_get_time);

	$result = mysqli_data_insert($tbL158,$pst_field,$pst_query);

	if(isset($result) && $result == 2) {
		
		$shF_query = array("userid"=>$cashier,"shiftid"=>$current_shift,"datelogged"=>$server_get_date,"isreversed"=>0,"deletedata"=>0);
		$shF = mysqli_data_checkr($tbL100,'(*)',$shF_query);
						
		if($shF == true) {
			
			$additionalQuery = " GROUP BY posid";
			$shFx = mysqli_data_fetch($tbL100,'posid',$shF_query,'array');
			$sqlset = "SUM(bill_amount)"; $wgt_gc_funds = 0;
			
			foreach($shFx as $k => $v) {
				$queryset = "posid={$v['posid']} AND shiftid={$current_shift} AND cashier={$cashier} AND media IN(1) AND datelogged='{$server_get_date}' AND isreversed=0 AND deletedata=0";
				...
				if($wgt_gc_funds >= 1) {
					...
					$acct_src = idget_data($tbL14,$v['posid'],'posname');
					$pst_query = array("counterid"=>$v['posid'],"shiftid"=>$current_shift,"userid"=>$cashier,"datelogged"=>$server_get_date);
					$pst_field = array("account"=>$acct_src,"shift_start_date"=>$start_end_date,"shift_end_date"=>$server_get_date,"shift_start_time"=>$start_time,"shift_end_time"=>$server_get_time,"shift"=>$shift,"counterid"=>$v['posid'],"shiftid"=>$current_shift,"userid"=>$cashier,"paid_amount"=>$wgt_gc_funds,"account_src"=>"pos","datelogged"=>$server_get_date,"timelogged"=>$server_get_time);
...
<div id="section-to-print" class="block-element">
	...
	<div class="block-element sml-rounded-button top-push-30 noscroll">
		<table cellpadding="0" cellspacing="0">
			<tr>
				<th width="250px" class="box-border-thick-right" align="center">Pay Types</th>
...
<script>
	...
	function wdr() {
		...
		vhtml = '';
		vhtml += '<form action="" method="post" autocomplete="off">';
		vhtml += '<input type="hidden" name="uri" value="apply-withdraw-fund">';
		vhtml += '<div class="pads10 alignlt">';
		vhtml += '<h3 class="large nobold">Note: This amount will deduct from your opening balance amount</h3><br>';
		vhtml += '<input type="text" name="float" id="float" placeholder="0.00">';
		vhtml += '</div>';
		vhtml += '<div class="top-pull-10 motion">';
		vhtml += '<input type="submit" name="withdrawbutton" value="Withdraw" class="nc-width-100 dark-black-white-state top-pull-15 bottom-pull-15 nunito-semibold rounded-button anchor ft-mini-size letter-spacing-2">';
	...
	function ctrf() {
		...
		vhtml += '</div>';
		vhtml += '<div class="top-pull-10 motion">';
		vhtml += '<input type="submit" name="applybutton" id="applybutton" value="Apply Transfer" class="nc-width-100 dark-black-white-state top-pull-15 bottom-pull-15 nunito-semibold rounded-button anchor ft-mini-size letter-spacing-2">';
		vhtml += '<p class="top-pull-15 alignct"><a href="javascript://" class="black-font" title="Close" onclick="cancelPrSign()">Cancel x</a></p>';
		vhtml += '</div>';
		vhtml += '</form>';
	...
	function addFund() {
		...
		vhtml = '';
		vhtml += '<form action="" method="post" autocomplete="off">';
		vhtml += '<input type="hidden" name="uri" value="apply-fund-to-float">';
		vhtml += '<div class="pads10 alignlt">';
		vhtml += '<h3 class="large nobold">Note: This amount will enter your opening balance amount</h3><br>';
		vhtml += '<input type="text" name="float" id="float" placeholder="0.00">';
		vhtml += '</div>';
		vhtml += '<div class="top-pull-10 motion">';
		vhtml += '<input type="submit" name="addbutton" value="Add" class="nc-width-100 dark-black-white-state top-pull-15 bottom-pull-15 nunito-semibold rounded-button anchor ft-mini-size letter-spacing-2">';
		vhtml += '<p class="top-pull-15 alignct"><a href="javascript://" class="black-font" title="Close" onclick="cancelPrSign()">Cancel x</a></p>';
		vhtml += '</div>';
		vhtml += '</form>';
	...
	function jbtrigger(e) {
		e.preventDefault();
		htmlFormField('applybutton','button');
		htmlpassval('Transferring..','applybutton');
	...
	function print_counter() {
		popmodalframe('frontdesk','recreation_user_counter_sheet',1,0,1000,2500);
	}
