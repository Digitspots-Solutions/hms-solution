...
<div class="nc-width-100 x-scroll bottom-push-20">
	<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
		...
		<span class="block-element bottom-push-10">
			<input type="checkbox" name="currentpayment" id="currentpayment" value="Yes"<?php if(isset($_POST['currentpayment']) && !empty($_POST['currentpayment']) && $_POST['currentpayment'] === 'Yes'): ?> checked="checked"<?php endif; ?>>&nbsp; <b class="ft-sml-size light-red-font nobold">Tick this box for current payment only</b>
		</span>
		...
		<span class="ln-display-box float-left cs-width-150 right-push-15">
			...
			<select name="recreationtype" id="recreationtype">
				<?php if(isset($_POST['recreationtype'])) { ?><option value="<?php echo $_POST['recreationtype']; ?>"><?php echo $_POST['recreationtype']; ?></option><?php } ?>
...
<div id="section-to-print" class="block-element pads15">
	...
	<?php
		...
		if(isset($_POST['searchbutton'])) {
			...
			if(isset($_POST['currentpayment']) && !empty($_POST['currentpayment']) && $_POST['currentpayment'] === 'Yes') {
				
				$onlycurpay = 1;
				$rcr_date = " AND datelogged >= '{$_POST['startdate']}' AND datelogged <= '{$_POST['enddate']}'";
				$py_sql = "SELECT recreation_number FROM $tbL107 WHERE deletedata=0".$rcr_date;
				$py_dataset = wgetSQL($py_sql);
				
				if(is_array($py_dataset) && count($py_dataset) > 0) {
					$rcr_payer = ""; $rcrnos = "";
					foreach($py_dataset as $ky => $vl) { $rcrnos .= "'".$vl['recreation_number']."',"; }
					$rcrnos = substr_replace($rcrnos,'',-1,1); $rcr_payer = " AND recreation_number IN({$rcrnos})";

					$sql = "SELECT * FROM $tbL105 WHERE deletedata=0".$rcr_type.$rcr_plan.$rcr_reg.$rcr_status.$rcr_duration.$rcr_payer;

				} else {
					$onlycurpay = 0;
					$rcr_date = "";
					$rcr_payer = " AND datelogged >= '{$server_get_date}' AND datelogged <= '{$server_get_date}'";
					$sql = "SELECT * FROM $tbL105 WHERE deletedata=0".$rcr_type.$rcr_plan.$rcr_reg.$rcr_status.$rcr_duration.$rcr_payer;
				}
			...
			?>
				...
				<table cellpadding="3" cellspacing="0">
					<tr>
						...
						<td class="alignct default-text-font-bold">Payment Date</td>
					...
					<?php
						if(is_array($dataset) && count($dataset) > 0) {
							...
							foreach($dataset as $key => $val) {
								...
								if(isset($onlycurpay) && $onlycurpay > 0) {
									$amt_sql = "SUM(amount)"; $amt_query = "memberid={$val['id']} AND deletedata=0".$rcr_date;
								} else {
								...
								$dt_sql = "SELECT datelogged FROM $tbL107 WHERE memberid={$val['id']} AND deletedata=0".$rcr_date." ORDER BY id DESC LIMIT 1"; $dt_dataset = wgetSQL($dt_sql); $payday = $dt_dataset[0]['datelogged'];
								...
								?>
									<tr>
										...
										<td class="alignct"><?php echo date("d/m/Y",strtotime($payday)); ?></td>
							...
							?>
								<tr>
									...
									<td class="alignct default-text-font-bold">&#8358; <?php echo number_format($total_rcr_amount,2); ?></td><td class="alignct">&nbsp;</td><td class="alignct">&nbsp;</td>
								</tr>
							<?php
						}
					?>
