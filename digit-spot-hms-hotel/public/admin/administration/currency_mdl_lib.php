...
<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		...
		&nbsp; Note: add new currency by clicking <u>add currency</u> button
 	</span>
...
<?php
	...
	$currency = array("Nigeria Naira"=>"NGN",
	...
	{
		createDatabasetable($var_tbl_24); //create a table for this post

		$fieldset1 = escape_data($_POST['fieldset1']);
		
		foreach ($currency as $cckey => $ccvalue) {
			if($fieldset1 == $cckey) {
				$fieldset2 = $ccvalue;
				break;
			}
		...
		$insert_dataproperty = array("currencyname"=>$fieldset1,"shortname"=>$fieldset2);
		$insert_constrain = "";
		$data_inserted = mysqli_data_insert($tbL26,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Add new currency type","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
				...
				<div class="nc-width-40">
					<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
						<div class="bottom-push-20 alignlt">
							<h3 class="nomargin">Country Currency</h3>
						</div>
						<span class="block-element bottom-push-10">
							<select name="fieldset1" id="fieldset1" required="required">
								...
								<?php
									foreach ($currency as $incckey => $inccvalue) {
										?>
											<option value="<?php echo $incckey; ?>"><?php echo $incckey; ?></option>
										<?php
									}
								?>
							</select>
	...
	if(isset($_POST['editbutton']))
	{
		...
		$data_inserted = mysqli_data_update($tbL26,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Change currency type","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['deletebutton']) && isset($_POST['checkers']))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$del = mysqli_data_update($tbL26,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Remove from currency list","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if((isset($_POST['searchbutton'])) && (isset($_POST['search']) && !empty($_POST['search']))) {
		$keywords=" AND (currencyname LIKE '".escape_data($_POST['search'])."%' OR shortname LIKE '".escape_data($_POST['search'])."%')";
	} else { 
		$keywords="";
	}
	...
	$dataproperty = "id,currencyname,shortname,status";
	$constrain = array("deletedata"=>0);
	$row = "array";

	$dataCollect = mysqli_data_fetch($tbL26,$dataproperty,$constrain,$row);

	if(is_array($dataCollect))
	{
		$thproperty = array("noth","country currency","short name","status","noth","enoth");
		...
		foreach($dataCollect as $theader => $tdata)
		{
			...
			$htmlresult .= '<td width="200px" align="center" class="box-border-thick-right">'.$tdata["currencyname"].'</td>';
			$htmlresult .= '<td width="200px" align="center" class="box-border-thick-right">'.$tdata["shortname"].'</td>';
			$htmlresult .= '<td width="150px" align="center" class="box-border-thick-right forest-green-font">'.$tdata["status"].'</td>';
			...
			if((isset($_GET['edit']) && $_GET['edit'] >= 1) && ($_GET['edit'] == $dataid))
			{
				...
				$htmlresult .= '<h4 class="large blue-font">Updating '.$tdata["currencyname"].'</h4><br>';
				$htmlresult .= '<div class="nc-width-40">';
				$htmlresult .= '<span class="block-element bottom-push-10">';
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Country Currency</small>';
				$htmlresult .= '<select name="fieldset1" id="fieldset1" required="required">';
				$htmlresult .= '<option value="" selected="selected">Choose Currency</option>';
								
				foreach ($currency as $incckey => $inccvalue) {
					$htmlresult .= '<option value="'.$incckey.'">'.$incckey.'</option>';
				}
