...
<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		...
		&nbsp; Note: create new source of business by clicking <u>new source</u> button
 	</span>
...
<?php
	...
	{
		createDatabasetable($var_tbl_41); //create a table for this post
		...
		$insert_constrain = "";
		$data_inserted = mysqli_data_insert($tbL43,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Create new source of business","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
				...
				<div class="nc-width-40">
					<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
						<div class="bottom-push-20 alignlt">
							<h3 class="nomargin">Creating New Source</h3>
						</div>
	...
	if((isset($_POST['statusbutton']) && isset($_POST['checkers'])) && (isset($_POST['cstatus']) && !empty($_POST['cstatus'])))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$cstat = mysqli_data_update($tbL43,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Change source of business status","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['editbutton']))
	{
		...
		$data_inserted = mysqli_data_update($tbL43,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Edit source of business details","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['deletebutton']) && isset($_POST['checkers']))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$usr_key = array("id"=>$fkey);
			$del = mysqli_data_update($tbL43,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Remove from source of business list","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	$dataCollect = mysqli_data_fetch($tbL43,$dataproperty,$constrain,$row);

	if(is_array($dataCollect))
	{
		$thproperty = array("noth","source","description","status","noth","enoth");
		...
		foreach($dataCollect as $theader => $tdata)
		{
			...
			if((isset($_GET['edit']) && $_GET['edit'] >= 1) && ($_GET['edit'] == $dataid))
			{
				...
				$htmlresult .= '<small class="block-element bottom-push-3 dark-grey-font left-pull-5">Source</small>';
