...
<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		...
		&nbsp; Note: indicate hotel invoice sequence label
 	</span>
...
<?php
	...
	{
		createDatabasetable($var_tbl_73); //create a table for this post
		...
		if(isset($fieldset2) && $fieldset2 >= 1) {
			...
			$data_inserted = mysqli_data_update($tbL77,$insert_dataproperty,$insert_constrain);
		} else {
			$insert_constrain = "";
			$data_inserted = mysqli_data_insert($tbL77,$insert_dataproperty,$insert_constrain);
		}
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Update hotel invoice sequence","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');

			$post_result .= '<span class="red-font">Hotel invoice sequence was updated successfully</span>';
		}
	...
	$hotel_query = '';
	$get_hotel_data = mysqli_data_fetch($tbL77,'id,prefixtext',$hotel_query,'noarray');
	...
	<div class="nc-width-40">
		<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
			<span class="block-element bottom-push-10 box-border-thick pads15 sml-rounded-button">
				<small class="block-element bottom-push-3 blue-font left-pull-5 alignlt"><b>HOTEL INVOICE SEQUENCE</b></small>
