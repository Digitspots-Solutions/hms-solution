...
<?php if(isset($_GET['agentuser']) && is_numeric($_GET['agentuser'])) { $myfirm = $_GET['agentuser']; } else { $myfirm = 0; } ?>

<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		<a href="?logs=<?php echo $logs; ?>&agentuser=<?php echo $myfirm; ?>" title="Refresh"><img src="<?php echo DOMAIN_URL; ?>theme/images/general/refresh.png"></a>
		&nbsp; Note: create new agent user by clicking <u>new user</u> button. All asterik or marked fields are compulsory
 	</span>
 	<span class="ln-display-box float-right">
		<a href="?logs=<?php echo $logs; ?>&c=new&agentuser=<?php echo $myfirm; ?>" class="submit pads12 sml-rounded-button blue-theme white-font">
		New User
		</a>
		<?php
...
<?php
	...
	$get_corporate_name = select_dt_fetch('status','Active',$tbL66,'id','name');

	#-----------------------------------------------------------------------------------------------------------------

	if(isset($_POST['submitbutton']))
	{
		createDatabasetable($var_tbl_67); //create a table for this post
		...
		$data_inserted = mysqli_data_insert($tbL71,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Create new agent user","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
				...
				<div class="nc-width-50">
					<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
						<div class="bottom-push-20 alignlt">
							<h3 class="nomargin">Creating New Agent User</h3>
						</div>
							...
							<div class="ln-display-box float-left nc-width-90">
								<select name="fieldset6" id="fieldset6" required="required">
									...
									if(isset($myfirm) && $myfirm >= 1) {
										...
										$currentcorporate = idget_data($tbL66,$corporateid,'name');
										?>
	...
	if((isset($_POST['statusbutton']) && isset($_POST['checkers'])) && (isset($_POST['cstatus']) && !empty($_POST['cstatus'])))
	{
		...
		foreach ($_POST['checkers'] as $fkey) {
			...
			$cstat = mysqli_data_update($tbL71,$usr_datasets,$usr_key);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Change agent user status","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['resetbutton']))
	{
		...
		$data_inserted = mysqli_data_update($tbL71,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Change agent user password","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['editbutton']))
	{
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>"Edit agent user details","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	$dataCollect = mysqli_data_fetch($tbL71,$dataproperty,$constrain,$row);
	...
	if(is_array($dataCollect))
	{
		...
		foreach($dataCollect as $theader => $tdata)
		{
			...
			$corporate = idget_data($tbL66,$tdata["corporatename"],'name');
