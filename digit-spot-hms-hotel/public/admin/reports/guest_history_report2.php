	...
	<div class="x-scroll">
		<div id="section-to-print" class="cs-width-1500">
			...
			<?php
				...
				$queryset = "t1.deletedata=0".$keywords;
				//$dataset = "SELECT t1.salutation,t1.fname,t1.lname,t1.booking_number,CONCAT(t2.checkin_date,t2.checkout_date) AS 'stayduration',COUNT(t2.roomid) AS 'noofrooms',t2.status,(SUM(t3.room_amount + t3.tax_amount + t3.consumption_tax_amount + t3.service_charge) - t3.discount_amount) AS 'totalamount' FROM {$tbL102} t1, {$tbL127} t2, {$tbL134} t3 WHERE t1.primary_guest=1 AND t1.booking_number=t2.booking_number AND t1.booking_number=t3.booking_number AND ".$queryset;

				$dataset = "SELECT t1.booking_number,t2.salutation,t2.fname,t2.lname,t1.checkin_date,t1.checkout_date,t3.status,COUNT(t3.roomid) AS 'noofrooms' FROM {$tbL130} t1, {$tbL102} t2, {$tbL127} t3 WHERE t2.primary_guest=1 AND t1.booking_number=t2.booking_number AND t1.booking_number=t3.booking_number AND ".$queryset;
