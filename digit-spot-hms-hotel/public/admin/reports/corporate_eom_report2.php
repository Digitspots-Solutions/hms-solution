	...
	if($show == true) {
		...
		if(is_array($dataCollect)) {
			...
			foreach($dataCollect as $theader => $tdata) {
				...
				$queryset2 = "cspgid={$dataid} AND transaction_type='Credit' AND datelogged >= '{$startdate}' AND datelogged <= '{$enddate}'"; $total_paid = mysqli_arithmetic_data($tbL63,$sqlset,$queryset2);

				if($tdata["creditlimit"] < 0) {
					$opening_bal = $tdata["creditlimit"] + $total_consumed;
					$closing_bal = $tdata["creditlimit"];
				} elseif($tdata["creditlimit"] >= 0) {
					$opening_bal = $tdata["creditlimit"];
					$closing_bal = $opening_bal;
				}

				//$balance_now = $opening_bal - $total_consumed;
				...
				$htmlresult .= '<td width="100px" align="center" class="box-border-thick-right">&#8358; '.number_format($total_paid,2).'</td>';
