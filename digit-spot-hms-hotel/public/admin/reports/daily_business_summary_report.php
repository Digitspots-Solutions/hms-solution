		...
		<div class="nc-width-100">
			<div id="section-to-print">
				...
				<div class="bottom-push-15" align="center">
					...
					<h3 class="large nobold default-text-font-bold">Daily Business Summary Report (between <?php echo $startdate; ?> and <?php echo $endate; ?>)</h3>
				</div>
				...
				<?php
					...
					$query3 = "TRIM(booking_number) <> '' AND TRIM(room_type_id) <> '' AND TRIM(roomid) <> '' AND room_type_id > 0 AND roomid > 0 AND room_status IN('CheckedIn') AND deletedata=0 AND bill_date BETWEEN '{$sdt}' AND '{$edt}'";
					$query3x = "status IN('CheckedOut') AND deletedata=0 AND checkin_date='{$sdt}' AND checkout_date='{$sdt}'";
					$data3 = "COUNT(roomid) AS 'totaloccupied'";
					$result3 = pfetch($data3,$tbL134,$query3);
					$result3x = pfetch($data3,$tbL127,$query3x);
					//$totalrooms3 = $result3[0]['totaloccupied'] - $result3x[0]['totaloccupied'];
					...
					$query4 = "charge_type IN('complimentary') AND room_status IN('CheckedIn') AND deletedata=0 AND bill_date BETWEEN '{$sdt}' AND '{$edt}'";
					$data4 = "COUNT(roomid) AS 'totalcompl'";
					$result4 = pfetch($data4,$tbL134,$query4);
					...
					$qSales1 = "SELECT SUM(room_amount) AS 'totalroomAmt', SUM(discount_amount) AS 'totaldiscountAmt', SUM(tax_amount) AS 'totaltaxAmt', SUM(consumption_tax_amount) AS 'totalconsumptionAmt', SUM(service_charge) AS 'totalserviceAmt' FROM {$tbL134} WHERE room_status IN('CheckedIn') AND charge_type NOT IN('complimentary') AND bill_date BETWEEN '{$sdt}' AND '{$edt}' AND deletedata=0"; $sales1 = wgetSQL($qSales1);
					...
					$qdayUseSales = "SELECT SUM(room_amount) AS 'totalroomAmt', SUM(discount_amount) AS 'totaldiscountAmt', SUM(tax_amount) AS 'totaltaxAmt', SUM(consumption_tax_amount) AS 'totalconsumptionAmt', SUM(service_charge) AS 'totalserviceAmt' FROM {$tbL134} t1 LEFT JOIN {$tbL127} t2 ON t1.roomid=t2.roomid WHERE checkin_date=checkout_date AND t2.status IN('CheckedOut') AND t1.bill_date BETWEEN '{$sdt}' AND '{$edt}' AND t1.deletedata=0";
					$dayUseSales = wgetSQL($qdayUseSales);
					...
					#individual
					$qmkSA1x = "SELECT COUNT(roomid) AS 'totalrooms' FROM {$tbL134} WHERE charge_type='individual' AND room_status IN('CheckedIn') AND bill_date BETWEEN '{$sdt}' AND '{$edt}' AND deletedata=0";
					$mkSA1x = wgetSQL($qmkSA1x);
					...
					$qmkSA1 = "SELECT SUM(room_amount) AS 'totalroomAmt', SUM(discount_amount) AS 'totaldiscountAmt', SUM(tax_amount) AS 'totaltaxAmt', SUM(consumption_tax_amount) AS 'totalconsumptionAmt', SUM(service_charge) AS 'totalserviceAmt' FROM {$tbL134} WHERE charge_type='individual' AND room_status IN('CheckedIn') AND bill_date BETWEEN '{$sdt}' AND '{$edt}' AND deletedata=0";

					$mkSA1 = wgetSQL($qmkSA1);
					...
					#corporate
					$qmkSA2x = "SELECT COUNT(roomid) AS 'totalrooms' FROM {$tbL134} WHERE charge_type='corporate' AND room_status IN('CheckedIn') AND bill_date BETWEEN '{$sdt}' AND '{$edt}' AND deletedata=0";
					$mkSA2x = wgetSQL($qmkSA2x);
					...
					$qmkSA2 = "SELECT SUM(room_amount) AS 'totalroomAmt', SUM(discount_amount) AS 'totaldiscountAmt', SUM(tax_amount) AS 'totaltaxAmt', SUM(consumption_tax_amount) AS 'totalconsumptionAmt', SUM(service_charge) AS 'totalserviceAmt' FROM {$tbL134} WHERE charge_type='corporate' AND room_status IN('CheckedIn') AND bill_date BETWEEN '{$sdt}' AND '{$edt}' AND deletedata=0";

					$mkSA2 = wgetSQL($qmkSA2);
					...
					#complimentary
					$qmkSA3x = "SELECT COUNT(roomid) AS 'totalrooms' FROM {$tbL134} WHERE charge_type='complimentary' AND room_status IN('CheckedIn') AND bill_date BETWEEN '{$sdt}' AND '{$edt}' AND deletedata=0";
					$mkSA3x = wgetSQL($qmkSA3x);
					...
					$qmkSA3 = "SELECT SUM(room_amount) AS 'totalroomAmt', SUM(discount_amount) AS 'totaldiscountAmt', SUM(tax_amount) AS 'totaltaxAmt', SUM(consumption_tax_amount) AS 'totalconsumptionAmt', SUM(service_charge) AS 'totalserviceAmt' FROM {$tbL134} WHERE charge_type='complimentary' AND room_status IN('CheckedIn') AND bill_date BETWEEN '{$sdt}' AND '{$edt}' AND deletedata=0";

					$mkSA3 = wgetSQL($qmkSA3);
					...
					#online booking
					$qmkSA4x = "SELECT COUNT(roomid) AS 'totalrooms' FROM {$tbL134} t1 LEFT JOIN {$tbL130} t2 ON t1.booking_number=t2.booking_number WHERE charge_type IN('corporate','individual') AND booking_src='Online' AND room_status IN('CheckedIn') AND bill_date BETWEEN '{$sdt}' AND '{$edt}' AND t1.deletedata=0";
					$mkSA4x = wgetSQL($qmkSA4x);
					...
					$qmkSA5 = "SELECT SUM(amount) AS 'totalrebate' FROM {$tbL163} WHERE approval_status IN('Approved','Completed') AND datelogged BETWEEN '{$sdt}' AND '{$edt}'";

					$mkSA5 = wgetSQL($qmkSA5);
					...
					$mkSA_total = $mkSA_ind + $mkSA_corp + $mkSA_agent + $mkSA_online + $mkSA_cancelled - ($mkSA_rebate + $mkSA_agent_commission);
					...
					$qRSv1 = "SELECT COUNT(roomid) AS 'totalrooms' FROM {$tbL127} WHERE datelogged BETWEEN '{$sdt}' AND '{$edt}' AND reservation='Reserving'"; $RSv1 = mysqli_data_array('assoc',$qRSv1);

					$qRSv2 = "SELECT COUNT(roomid) AS 'totalrooms' FROM {$tbL127} WHERE datelogged BETWEEN '{$sdt}' AND '{$edt}' AND status IN('No Show')"; $RSv2 = mysqli_data_array('assoc',$qRSv2);

					$qRSv3 = "SELECT COUNT(roomid) AS 'totalrooms' FROM {$tbL127} WHERE datelogged BETWEEN '{$sdt}' AND '{$edt}' AND status IN('Cancelled')"; $RSv3 = mysqli_data_array('assoc',$qRSv3);

					$qRSv4 = "SELECT COUNT(roomid) AS 'totalrooms' FROM {$tbL127} WHERE checkin_date BETWEEN '{$sdt}' AND '{$edt}' AND reservation='Reserving' AND status IN('CheckedIn')"; $RSv4 = mysqli_data_array('assoc',$qRSv4);

					$qRSv5 = "SELECT COUNT(roomid) AS 'totalrooms' FROM {$tbL127} t1 LEFT JOIN {$tbL130} t2 ON t1.booking_number=t2.booking_number WHERE booking_src='InPerson' AND t2.checkin_date BETWEEN '{$sdt}' AND '{$edt}' AND status IN('CheckedIn')";
					$RSv5 = mysqli_data_array('assoc',$qRSv5);

					$qRSv6 = "SELECT COUNT(roomid) AS 'totalrooms' FROM {$tbL134} WHERE bill_date BETWEEN '{$sdt}' AND '{$edt}' AND room_status IN('CheckedIn')"; $RSv6 = mysqli_data_array('assoc',$qRSv6);

					$qRSv7 = "SELECT COUNT(roomid) AS 'totalrooms' FROM {$tbL127} WHERE checkout_date BETWEEN '{$sdt}' AND '{$edt}' AND status IN('CheckedOut')"; $RSv7 = mysqli_data_array('assoc',$qRSv7);
							...
							<td class="right-pull-10 left-pull-10 box-border-thick">
								<table cellpadding="3" cellspacing="0">
									<tr>
										...
										<td class="alignct"><?php echo $startdate; ?> - <?php echo $endate; ?></td>
										<td class="alignct">%</td>
									</tr>
									<tr>
						...
						<tr>
							...
							<td class="">
								<table cellpadding="3" cellspacing="0">
									...
									<tr>
										<td class="right-pull-10 left-pull-10 box-border-thick alignrt"><?php echo $totalrooms3 - $mkSA_compl_rooms; ?></td>
										<td class="right-pull-10 left-pull-10 box-border-thick alignrt">0</td>
