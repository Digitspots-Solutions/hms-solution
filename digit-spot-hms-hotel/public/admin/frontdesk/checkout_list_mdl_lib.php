...
<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		...
		&nbsp; Note: here you can get the list of check-out guests
 	</span>
				...
				<?php
					...
					if(isset($_POST['bookingtype']) && !empty($_POST['bookingtype'])) {
						...
						if(isset($_POST['cspg']) && !empty($_POST['cspg'])) {
							$for_bkg = "SELECT booking_number FROM {$tbL130} WHERE booking_type='{$_POST['bookingtype']}' AND bill_to_g='{$_POST['cspg']}' AND reservation IN('Checking Out')";
						} else {
							$for_bkg = "SELECT booking_number FROM {$tbL130} WHERE booking_type='{$_POST['bookingtype']}' AND reservation IN('Checking Out')";
						}
					...
					$queryset = "deletedata=0 AND status='CheckedOut'".$keywords." ORDER BY id DESC";
					...
					$keys = array(
						...
						"checkout_date"=>"(df)check-out date",
						"checkout_time"=>"check-out time",
						"checkout_byuser"=>"check-out by"
					);
...
<script>
	...
	function getcspg(val) {
		...
		if(val == 'Corporate') {
			sqldatastring.sql = "SELECT a.bill_to_g, b.name FROM booking_invoice_tbl a, cspg_tbl b WHERE a.reservation IN('Checking Out') AND a.bill_to_g=b.id AND a.deletedata=0 GROUP BY a.bill_to_g ORDER BY b.name ASC";
