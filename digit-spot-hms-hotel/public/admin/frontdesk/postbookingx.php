...
if($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if((isset($_POST['postdatarequest']) && isset($_POST['kyw'])) && ($_POST['kyw'] == 'modifybooking')) {
		...
		//if(isset($isupdated) && $isupdated == 2) {
			...
			if(isset($th_biller) && $th_biller > 0) {
				...
				if($wgt_bill_type == "Corporate") {
					...
					if(!empty($get_outlet_charges) && $get_outlet_charges > 0) {
						...
						$transaction_desc = "Migrated outlet bills for booking number: ".$wgt_booking_number;
			...
			$islogfile = 1;
			$logfile_msg = "Recently modified this booking (booking number: ".$wgt_booking_number.") to ".$wgt_booking_type." type";

			$isguestAct = 1;
			$pst_booking_number = $wgt_booking_number;
			$guestAct_msg = "Guest booking recently modified to ".$wgt_booking_type." type: ".$wgt_remarks;
