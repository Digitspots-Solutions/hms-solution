<?php
	...
	$guest_no = explode('-', $guest_no); $var_bkgs = "";

	foreach($guest_no as $bkg) {
		if(!empty($bkg)) {
			$var_bkgs .= "'".$bkg."',";
		}
	}

	$var_bkgs = substr_replace($var_bkgs,'',-1,1);
			...
			<div class="block-element top-push-3 bottom-push-15 box-border-thick sml-rounded-button noscroll">
				<table cellpadding="0" cellspacing="0" class="ft-xxsml-size default-text-font-bold">
					...
					<?php
						...
						$additionalQuery = " AND booking_number IN({$var_bkgs}) AND status NOT IN('No Show','Cancelled','Reserved')";
						$datasets="booking_number,roomid,checkin_date,checkout_date,datelogged,status";
						$booking_query2 = array("deletedata"=>0);
