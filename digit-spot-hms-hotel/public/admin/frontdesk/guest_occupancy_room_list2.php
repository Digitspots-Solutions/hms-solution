<table cellpadding="0" cellspacing="0">
	<tr>
		<th width="50px" align="center"><input type="checkbox" name="checkall" id="checkall" lang="off" onclick="docA(this)"></th>
	...
	<?php
		...
		if($wgt_balance <= 0) { $isroomcredited = true; }
		else { $wgt_balance = false; }
		...
		foreach($get_occupancy_data as $gokey => $govalue) {
			...
			if(isset($govalue['cancel_policy']) && $govalue['cancel_policy'] >= 1) {
				...
				if(isset($govalue['status']) && ($govalue['status'] == 'CheckedOut' || $govalue['status'] == 'Cancelled' || $govalue['status'] == 'Swapped' || $govalue['status'] == 'Upgraded' || $govalue['status'] == 'Downgraded' || $govalue['status'] == 'No Show')) {
					...
					$disabled_this_room="class='allow-check'"; $disabled_color = "";
				}
			}
			...
			?>
				<tr class="<?php echo $disabled_color; ?>">
					<td width="50px" align="center">
						<input type="checkbox" name="checkers[]" id="chk<?php echo $checker_id; ?>" value="<?php echo $govalue['id']; ?>" onclick="roomChk('chk<?php echo $checker_id; ?>'); buttonCtrl(this.lang)" lang="<?php echo $govalue['status']; ?>" title="<?php echo $room_prefix.$room_number; ?>" <?php echo $disabled_this_room; ?>>
					</td>
					<td width="150px" align="left" class="right-pull-5">
						<?php if($govalue['status'] == 'Reserved'): ?>

							<select name="roomtype[]" id="roomtype<?php echo $govalue['id']; ?>" class="nopads" onchange="getdata('roomnumber<?php echo $govalue['id']; ?>','eget-rooms','roomtype<?php echo $govalue['id']; ?>','dropbox')" required>
					...
					<td width="150px" align="left">
						...
						<?php if($govalue['status'] == 'Reserved'): ?>
							...
							<div class="right-pull-5"><select name="roomnumber[]" id="roomnumber<?php echo $govalue['id']; ?>" class="nopads"><?php if($isroomno == 1) { ?><option value="<?php echo $govalue['roomid']; ?>" selected><?php echo $room_prefix.$room_number; ?></option><?php } echo $getroomlist; ?></select></div>
					...
					<td width="50px" align="left" class="right-pull-3">
						<select name="adults[]" class="nopads" required="required">
					...
					<td width="50px" align="left" class="right-pull-3">
						<select name="childs[]" class="nopads" required="required">
					...
					<td width="80px" align="left" class="right-pull-3">
						<select name="occupancytype[]" class="nopads">
					...
					<td width="100px" align="left" class="left-pull-5">
						<input type="date" name="checkin[]" value="<?php echo $govalue['checkin_date']; ?>" style="font-size: 12px !important" class="nopads no-back-black"<?php echo $nodateselect; ?>>
					</td>
					<td width="100px" align="left" class="left-pull-5">
						<input type="date" name="checkout[]" value="<?php echo $govalue['checkout_date']; ?>" style="font-size: 12px !important" class="<?php echo $extendcolor; ?>" data-ext="<?php echo $govalue['roomid']; ?>" data-chkout="<?php echo $govalue['checkout_date']; ?>" lang="<?php echo $room_prefix.$room_number; ?> (<?php echo $block_name; ?>)" title="<?php echo $extendtitle; ?>" onclick="<?php echo $extendtrigger; ?>"<?php echo $nodateselect; ?>>
					</td>
					<td width="100px" align="left">
						<span class="ln-display-box float-left right-pull-7 ft-xsml-size"><?php echo $govalue['status']; ?></span><span class="ln-display-box float-left"><?php if(isset($govalue['status']) && $govalue['status'] == 'CheckedIn') { ?><img src="<?php echo DOMAIN_URL; ?>theme/images/general/checkin_card_icon.png" class="anchor" title="Check-In Card" onclick=""> <?php if($wgt_booking_type == 'individual') { ?><img src="<?php echo DOMAIN_URL; ?>theme/images/general/tariff_change_icon.png" class="anchor" title="Tariff Change" onclick="popmodalframe('frontdesk','manualtariffchange','<?php echo $booking_number; ?>',<?php echo $govalue['roomid']; ?>,500,500)"> <img src="<?php echo DOMAIN_URL; ?>theme/images/general/inclusions_icon.png" class="anchor" title="Inclusions" onclick=""><?php } } ?></span>
					...
					<td width="100px" align="left">
						<?php 
							...
							if(is_array($rsArry2)) {
								?>
									<a href="javascript:void(0)" class="ft-xsml-size blue-font" title="See bills allow to room" onclick="chgclass('bill<?php echo $govalue['id']; ?>','fx-position-flow cs-width-200 white-theme box-border-thick pads7 xsml-rounded-button right-push-30 motion')">Bill to</a>
								...
								?>
									<small class="ft-xxsml-size dark-grey-font">No bills allowed</small>
								<?php
							}
						?>
					</td>
					<td width="100px" align="center">
						<?php
							...
							if(isset($govalue['status']) && ($govalue['status'] == 'CheckedIn' || $govalue['status'] == 'CheckedOut')) { ?> <img src="<?php echo DOMAIN_URL; ?>theme/images/general/checkin_card_icon.png" class="anchor" title="Bill Status" onclick="popmodalframe('frontdesk','accountstatement','<?php echo $booking_number; ?>',<?php echo $govalue['roomid']; ?>,1000,2500)"><?php }
						?>
					</td>
					<td width="150px" align="center">
						<?php if($govalue['remarks'] == NULL || $govalue['remarks'] == '') { ?><a href="javascript:void(0)" class="blue-font ft-xxsml-size" onclick="popmodalframe('frontdesk','roomremark','<?php echo $booking_number; ?>','<?php echo $govalue['roomid']; ?>',600,400)">Remark</a><?php } else { ?><small class="ft-xxsml-size light-red-font block-element"><?php echo $govalue['remarks']; ?></small> <a href="javascript:void(0)" class="royal-blue-font ft-xxsml-size" onclick="popmodalframe('frontdesk','roomremark','<?php echo $booking_number; ?>','<?php echo $govalue['roomid']; ?>',600,400)">Update Remark</a><?php } ?> | <a href="javascript:void(0)" class="blue-font ft-xxsml-size" onclick="popmodalframe('frontdesk','applydiscount','<?php echo $booking_number; ?>','<?php echo $govalue['roomid']; ?>',500,400)" title="<?php if($govalue['isdiscount'] > 0) { ?>Discount applied<?php } else { ?>Apply<?php } ?>">Discount</a>
						<!--<ul class="nolist">
							<li></li>
							<li></li>
						</ul>-->
					...
					<td width="400px" align="left">
						<table cellpadding="0" cellspacing="0">
							<tr>
								...
								<td width="30px" align="center"><img src="<?php echo DOMAIN_URL; ?>theme/images/general/edit_record_icon.png" class="anchor" title="Edit Record" onclick="popmodalframe('frontdesk','guestdetail','<?php echo $booking_number; ?>','<?php echo $govalue['customerid']; ?>',1000,1500)"></td>
							</tr>
						</table>
					</td>
				</tr>
...
<script>
	...
	function docA(obj) {
		if(obj.lang == 'off') {
			...
			var getcheckers = document.getElementsByClassName('allow-check');
			var ischeckin = 0, isreserved = 0;
			
			for(var c=0; c < getcheckers.length; c++) {
				getcheckers[c].setAttribute('checked','checked');
				if(getcheckers[c].getAttribute('lang') == 'CheckedIn') { ischeckin += 1; }
				if(getcheckers[c].getAttribute('lang') == 'Reserved') { isreserved += 1; }
			}

			if(ischeckin > 0 && isreserved == 0) {
				document.getElementById('btn1').disabled = true;
				document.getElementById('btn2').disabled = true;
				document.getElementById('btn3').disabled = true;
				document.getElementById('btn4').disabled = true;
				document.getElementById('btn5').disabled = false;
			} else if(ischeckin == 0 && isreserved > 0) {
			...
			for(var c=0; c < getcheckers.length; c++) {
				getcheckers[c].removeAttribute('checked');
			}
