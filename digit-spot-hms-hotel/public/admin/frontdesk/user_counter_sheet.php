<?php
	...
	$queryset_sales = array("userid"=>$user,"counter_used"=>$counter,"shiftid"=>$shiftid,"isreversed"=>0);
	$dataset_sales = "sales_description,booking_number,receipt_number,biller,customerid,transaction_type,amount,datelogged,timelogged";
	...
	if(is_array($wget_sales) && count($wget_sales) > 0) {
		foreach($wget_sales as $key => $val) {
			...
			if($val['biller'] > 0) {
				$customer = idget_data($tbL58,$val['biller'],'name');
			} else {
				$customer = idget_data($tbL102,$val['customerid'],'fname');
				$customer .= ' ';
				$customer .= idget_data($tbL102,$val['customerid'],'lname');
			}

			if($val['transaction_type'] == 'credit' || $val['transaction_type'] == 'Credit') { $credit_amount = $val['amount']; }
			else { $credit_amount = 0; }

			if($val['transaction_type'] == 'refund' || $val['transaction_type'] == 'debit' || $val['transaction_type'] == 'rebate') { $debit_amount = str_replace('-','',$val['amount']); }
			else { $debit_amount = 0; }

			if($val['transaction_type'] == 'credit' || $val['transaction_type'] == 'Credit' || $val['transaction_type'] == 'refund' || $val['transaction_type'] == 'debit' || $val['transaction_type'] == 'rebate') {
				...
				$sales['desc'] = $val['sales_description'];
				$sales['credit'] = $credit_amount;
				$sales['debit'] = $debit_amount;
	...
	$additionalQuery = " AND paymode NOT IN(0) AND datelogged BETWEEN '{$start_date}' AND '{$end_date}'";
	$queryset_sales2 = array("userid"=>$user,"counter_used"=>$counter,"shiftid"=>$shiftid);
	$dataset_sales2 = "detail,transaction_number,cspgid,transaction_type,amount,datelogged,timelogged";
	$wget_sales2 = mysqli_data_fetch($tbL63,$dataset_sales2,$queryset_sales2,'array');

	if(is_array($wget_sales2) && count($wget_sales2) > 0) {
		foreach($wget_sales2 as $key => $val) {
			
			if($val['cspgid'] > 0) {
				$customer = idget_data($tbL58,$val['cspgid'],'name');
			} else {
				$customer = "";
			}
			...
			if($val['transaction_type'] == 'credit' || $val['transaction_type'] == 'Credit') {
				...
				$sales['date'] = date('d/m/Y',strtotime($val['datelogged']));
				$sales['time'] = $val['timelogged'];
				$sales['receipt'] = $val['transaction_number'];
				$sales['bookingno'] = "";
				$sales['guestname'] = $customer;
	...
	$additionalQuery = " AND datelogged BETWEEN '{$start_date}' AND '{$end_date}'";
	$queryset_sales3 = array("userid"=>$user,"isreversed"=>0,"deletedata"=>0);
	$dataset_sales3 = "recreation_number,memberid,amount,invoice_number,mode,detail,datelogged,timelogged";
	$wget_sales3 = mysqli_data_fetch($tbL107,$dataset_sales3,$queryset_sales3,'array');
	...
	<div class="right-pull-5 left-pull-5" align="center">
		...
		<table style="width: 900px !important; font-size: 11px !important;" cellpadding="2" cellspacing="0" border="1">
			...
			<?php
				...
				$totalcredit = 0; $totaldebit = 0; $numbr = 0;
			...
			?>
				...
				<tr>
					<td class="alignct default-text-font-bold"><?php echo $numbr; ?>.</td>
				...
				if(is_array($wget_counter) && count($wget_counter) > 0):
					foreach($wget_counter as $key => $val):
						...
						if($pay_type_gcr === 'Yes'): 
							...
							$gcr_ob = $gcr_ob + $val['openingbalance'];
							$gcr_wdr = $gcr_wdr + $val['withdrawal'];
						endif;
		...
		<?php $actual_gcr_amount = ($gcr_amount + $gcr_ob) - ($actual_refund + $gcr_wdr); ?>
