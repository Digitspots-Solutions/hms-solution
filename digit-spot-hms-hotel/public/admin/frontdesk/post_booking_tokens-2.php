<?php
	...
	if(isset($_POST['submitbutton'])) {
		...
		if(isset($_POST['wgtag']) && $_POST['wgtag'] == 'guestdetail') {
			...
			if(isset($isupdate) && $isupdate == 2) {
				...
				$ths_guest_pry = $wgt15;
				$guestAct_msg = "Guest detail with code: P".$wgt15." is changed";
			}
		}
		...
		if(isset($_POST['wgtag']) && $_POST['wgtag'] == 'billtoroomservice') {
			...
			if(isset($_POST['outlets'])) {
				...
				$new_bs = ""; foreach($_POST['outlets'] as $bs) { $new_bs .= $bs.','; }
				...
				if(isset($isupdate) && $isupdate == 2) {
					...
					$isguestAct = 1;
					$pst_booking_number = $wgt1;
					$guestAct_msg = "Bill-to-room service of this guest was changed as at ".$server_get_date." ".$server_get_time;
				}
			}
		}
	...
	if(isset($_POST['savechangesbutton']) && isset($_POST['checkers'])) {
		...
		if(isset($_POST["roomnumber$checkers"]) && !empty($_POST["roomnumber$checkers"]) && $_POST["roomnumber$checkers"] > 0) {
			...
			if($in_data[16] > 0 && $in_data[16] != $push_new_room) {
		...
		for($d=0; $d < $isday; $d++) {
			...
			if($chk_entry == true) {
				$pst_field = array("roomid"=>$push_new_room,"bill_date"=>$getDt,"weekday"=>strtolower($getWk));
		...
		if(isset($ocdata[0]) && !empty($ocdata[0]) && $ocdata[0] > 0) {
			$wgt_occupancy_charges = $ocdata[0];
			$sql_udata = array("room_type_id"=>$wgt_pst_roomtype,"occupancy_charges"=>$wgt_occupancy_charges);
			//$sql_udata = array("room_type_id"=>$wgt_pst_roomtype,"occupancy_charges"=>$wgt_occupancy_charges,"bill_date"=>$checkin,"weekday"=>$weekoftheday);
		} else {
			$sql_udata = array("room_type_id"=>$wgt_pst_roomtype);
			//$sql_udata = array("room_type_id"=>$wgt_pst_roomtype,"bill_date"=>$checkin,"weekday"=>$weekoftheday);
		}
		...
		$islogfile = 1;
		$logfile_msg = "Guest room detail for room number (".$room_prefix.$room_number.") and the booking number (".$booking_number.") changed by this user";
		
		$isguestAct = 1;
		$pst_booking_number = $booking_number;
		$guestAct_msg = "Guest room detail for room number (".$room_prefix.$room_number.") and the booking number (".$booking_number.") changed";
	...
	if(isset($_POST['cancelroombutton']) && isset($_POST['wgtcroom'])) {
		...
		if(isset($checkers) && $checkers >= 1) {
			...
			$rrquery = array("booking_number"=>$booking_number,"roomid"=>$room_id,"day"=>1);
	...
	if(isset($_POST['checkinbutton']) && isset($_POST['checkers'])) {
		...
		if((isset($checkers) && $checkers >= 1) && (isset($room_id) && $room_id >= 1) && (strtotime($server_get_date) >= strtotime($ischeckindate)) && (strtotime($ischeckoutdate) > strtotime($ischeckindate))) {
			...
			$islogfile = 1;
			$logfile_msg = "Guest room status for room number (".$room_prefix.$room_number.") and the booking number (".$booking_number.") changed to CheckedIn by this user";
			
			$isguestAct = 1;
			$pst_booking_number = $booking_number;
			$guestAct_msg = "Guest room status for room number (".$room_prefix.$room_number.") and the booking number (".$booking_number.") changed to CheckedIn upon guest arrival";
		...
		if(isset($checkers) && $checkers >= 1) {
			...
			if($gpos_data == false) {
				...
				if($checkedin_date == $server_get_date) {
					...
					if($chkiftr == false) {
						$additionalQuery = "";
						$sql_cquery = array("booking_number"=>$booking_number,"roomid"=>$room_id,"charge"=>"yes","ischarged"=>0,"deletedata"=>0); $sql_cdata = array("ischarged"=>1,"bill_time"=>$server_get_time);
	...
	if(isset($_POST['swaproombutton']) && isset($_POST['wgtroom']) && $_POST['wgtroom'] >= 1) {
		...
		if(isset($daysLeft) && $daysLeft > 0) {
			...
			$invdatasets = "invoice_number,room_amount,tax_amount,consumption_tax_amount,service_charge,charge,charge_type,billto,actual_room_amount,actual_tax_amount,actual_service_charge,actual_consumption_tax_amount,wkf";
	...
	if(isset($_POST['updownroombutton']) && isset($_POST['wgtroom']) && $_POST['wgtroom'] >= 1) {
		...
		if(isset($daysLeft) && $daysLeft > 0) {
			...
			$xwgt_tax = ($wgttax / 100) * $wgt_room_rate;
			$xwgt_consumption = ($wgtconsumption / 100) * $wgt_room_rate;
			$xwgt_service_charge = ($wgtservicecharge / 100) * $wgt_room_rate;
			$chargeroom = $rdata[2];
	...
	if(isset($_POST['roomtariffchangebutton']) && isset($_POST['wgtroom']) && $_POST['wgtroom'] >= 1) {
		...
		if($pst_booking_type == 'corporate' && $pst_bill_to > 0) {
			$weekday = date('l',strtotime($server_get_date));
			$weekday = strtolower($weekday);

			$json_constrain = array("corporateid"=>$pst_bill_to,"room_type_id"=>$wgtpstroomtype,"ratetype"=>"naira","day"=>$weekday,"status"=>"Active","deletedata"=>0);
			$corporate_price = mysqli_data_fetch($tbL147,'price',$json_constrain,'noarray');
			$rack_rate = $corporate_price[0];
		} else {
			$rack_rate = idget_data($tbL52,$wgtpstroomtype,'defaultprice');
		}
		...
		if(isset($daysLeft) && $daysLeft > 0) {
			...
			$thsStatus = "Swapped"; $actionChanged = "swap";

			$sql_uquery = array("booking_number"=>$wgtpstbk,"roomid"=>$wgtcuroomid);
			$sql_udata = array("housekeeping_stateid"=>$house_keeping_status,"room_status_id"=>$room_status,"status"=>$thsStatus);
			mysqli_data_update($tbL127,$sql_udata,$sql_uquery);
