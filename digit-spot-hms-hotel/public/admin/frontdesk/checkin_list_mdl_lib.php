...
<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		...
		&nbsp; Note: here you can get the list of check-in guests
 	</span>
				...
				<?php
					...
					if(isset($_POST['bookingtype']) && !empty($_POST['bookingtype'])) {
						...
						if(isset($_POST['cspg']) && !empty($_POST['cspg'])) {
							$iscspg=1;
							$for_bkg = "SELECT t1.booking_number,t2.id FROM {$tbL130} t1, {$tbL102} t2 WHERE t2.booking_type='{$_POST['bookingtype']}' AND t1.booking_type='{$_POST['bookingtype']}' AND t1.bill_to_g='{$_POST['cspg']}' AND t1.reservation IN('Checking In')";
						} else {
							$iscspg=0;
							$for_bkg = "SELECT booking_number FROM {$tbL130} WHERE booking_type='{$_POST['bookingtype']}' AND reservation IN('Checking In')";
						}
						...
						if(is_array($bkg)) {
							...
							$get_bkgnos = ""; $get_ccnos = "";

							if(isset($iscspg) && $iscspg == 1) {
								
								foreach($bkg as $key => $val) {
									$get_bkgnos .= "'".$val['booking_number']."',";
									$get_ccnos .= $val['id'].",";
								}
								
								$get_bkgno = substr_replace($get_bkgnos,'',-1,1);
								$get_ccno = substr_replace($get_ccnos,'',-1,1);

								$keywords .= " AND booking_number IN({$get_bkgno}) AND customerid IN({$get_ccno})";
					...
					if((isset($_POST['startdate']) && !empty($_POST['startdate'])) && (isset($_POST['endate']) && !empty($_POST['endate']))) {
						...
						$keywords .= " AND checkin_date BETWEEN '{$server_get_date}' AND '{$server_get_date}'";
					}
					...
					$queryset = "deletedata=0 AND status='CheckedIn'".$keywords." ORDER BY id DESC";
					...
					$keys = array(
						...
						"roomid"=>"room no.",
						"noofdays"=>"stay duration",
						"checkin_date"=>"(df)check-in date",
						"checkin_time"=>"check-in time",
						"checkin_byuser"=>"check-in by",
						"datelogged"=>"(df)booking date",
					);
...
<script>
	...
	function getcspg(val) {
		...
		if(val == 'Corporate') {
			sqldatastring.sql = "SELECT a.bill_to_g, b.name FROM booking_invoice_tbl a, cspg_tbl b WHERE a.reservation IN('Checking In') AND a.bill_to_g=b.id AND a.deletedata=0 GROUP BY a.bill_to_g ORDER BY b.name ASC";
