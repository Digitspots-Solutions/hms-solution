...
<div class="block-element bottom-push-30">
 	...
 	<span class="ln-display-box float-right">
		<?php if(isset($allowMgnCoupon) && $allowMgnCoupon == 200): ?>
		<a href="?logs=<?php echo $logs; ?>&c=new" class="submit pads12 sml-rounded-button blue-theme white-font">New Coupon</a>
		<?php endif; ?>
	</span>
...
<?php
	...
	if(isset($_POST['submitbutton']))
	{
		...
		$insert_dataproperty = array("coupon_code"=>$new_coupon_code,"guest_name"=>ucwords(strtolower($fieldset1)),"guest_contact"=>$fieldset2,"coupon_amount"=>$fieldset3,"payment_mode"=>$fieldset4,"coupon_type"=>0,"expires_on"=>$coupon_expiry_default_date,"customerid"=>0,"userid"=>$userSignedIn,"coupon_status"=>"Unused","datelogged"=>$server_get_date,"timelogged"=>$server_get_time);
		$insert_constrain = "";
		...
		if(isset($data_inserted) && $data_inserted == 2)
		{
			...
			/*$totalamount = $fieldset3;
			$wgt_paymentmode = $fieldset4;
			...
			if((isset($wgt_paymentmode) && $wgt_paymentmode > 0) && (isset($totalamount) && $totalamount > 0)) {
				...
				$sales_description = 'Coupon ('.$new_coupon_code.') manually created with '.idget_data($tbL24,$wgt_paymentmode,'name').' payment';

				$sql_py_pst_data = array("biller"=>0,"sales_point"=>"booking","booking_number"=>"N/A","receipt_number"=>"","customerid"=>0,"transaction_type"=>"coupon","amount"=>$totalamount,"payment_mode"=>0,"sales_description"=>$sales_description,"ispaid"=>1,"userid"=>$userSignedIn,"counter_used"=>$counter_sesid,"shiftid"=>$current_shift,"datelogged"=>$server_get_date,"timelogged"=>$server_get_time,"bizday"=>$server_get_bizid);
	...
	if(isset($_GET['refund']) && $_GET['refund'] >= 1) {
		...
		$coupon_id = escape_data($_GET['refund']);

		$coupon_code = idget_data($tbL129,$coupon_id,'coupon_code');
		$coupon_amount = idget_data($tbL129,$coupon_id,'coupon_amount');

		$pst_query = array("id"=>$coupon_id);
		$pst_field = array("coupon_status"=>"Refund","status"=>0,"usedby"=>$userSignedIn);
		$pst_result = mysqli_data_update($tbL129,$pst_field,$pst_query);

		if($pst_result == 2) {
			$post_result = "Coupon is now refunded successfully..";

			//create a log file
			$log_message = "Recently refunded this coupon amount to guest. Amount: {$coupon_amount} | Code: {$coupon_code}";
			$log_datasets = array("userid"=>$userSignedIn,"logcategory"=>$smdl,"message"=>$log_message,"datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
		}
	...
	if((isset($_POST['searchbutton'])) && (isset($_POST['search']) && !empty($_POST['search']))) {
		$keywords=" AND (guest_name LIKE '%".escape_data($_POST['search'])."%' OR coupon_code LIKE '".escape_data($_POST['search'])."%') ORDER BY id DESC";
	} else { 
		$keywords="ORDER BY id DESC";
	}
	...
	if(is_array($dataCollect))
	{
		...
		if(isset($allowMgnCoupon) && $allowMgnCoupon == 200) {
		...
		foreach($dataCollect as $theader => $tdata)
		{
			...
			} elseif($tdata["coupon_status"] == "Refund") {
				$operation_status = '<span class="block-element red-font">Refunded</span>';
			} elseif($tdata["coupon_status"] == "Unused") {
				$operation_status = '<a href="?logs='.$logs.'&refund='.$dataid.'&pg='.$curpage.'&start='.$pgstart.'&limit='.$pglimit.'" class="block-element blue-font add-bold">[Refund]</a>';
			}
			...
			if((isset($_GET['refund']) && $_GET['refund'] >= 1) && ($_GET['refund'] == $dataid && !empty($post_result)))
			{

				$htmlresult .= '<tr>';
				$htmlresult .= '<td colspan="11">';
				$htmlresult .= '<div id="tr" class="block-element grey-1-theme pads30 alignct light-red-font">';
				$htmlresult .= $post_result;
				$htmlresult .= '</div>';
				$htmlresult .= '</td>';
				$htmlresult .= '</tr>';
			}
