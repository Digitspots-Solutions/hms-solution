...
<?php
	...
	if(isset($allowSeeInbox) && $allowSeeInbox == 200) {
		$inbox_selection_key = array("deletedata"=>0,"archivedata"=>0);
	} else {
	...
	if(isset($inbox_count) && $inbox_count >= 1)
	{
		...
		$additionalQuery = " AND (user_one={$userSignedIn} OR user_two={$userSignedIn} OR user_three={$userSignedIn} OR user_four={$userSignedIn} OR user_five={$userSignedIn}) AND subject REGEXP '^PR'";
		$jba_query = array("approval_type"=>"IOU","approval_status"=>"Pending","deletedata"=>0);
		$datasets = "subject,user_one,approval_one,user_two,approval_two,user_three,approval_three,user_four,approval_four,user_five,approval_five,approval_status";
		$get_jba = mysqli_data_fetch($tbL151,$datasets,$jba_query,'array');

		if(is_array($get_jba)) {
			foreach($get_jba as $key => $val) {
			
				$iou_no = idget_fdata($tbL153,'pr_no',$val['subject'],'iou_no');

				if($val['user_one'] == $userSignedIn && $val['approval_one'] == 0) {
					
					$additionalQuery = " AND subject LIKE '%{$iou_no}%'";
					$inbox_query = array("receiver"=>$userSignedIn,"msgtype"=>9,"deletedata"=>0,"archivedata"=>0);
					$get_inbID = mysqli_data_fetch($tbL104,'id',$inbox_query,'noarray');
					
					$read_query = array("id"=>$get_inbID[0]);
					$post_data = array("read_status"=>2);
					
					mysqli_data_update($tbL104,$post_data,$read_query);

				} elseif($val['user_two'] == $userSignedIn && $val['approval_two'] == 0) {
				...
				} elseif($val['user_three'] == $userSignedIn && $val['approval_three'] == 0) {
				...
				} elseif($val['user_four'] == $userSignedIn && $val['approval_four'] == 0) {
				...
				} elseif($val['user_five'] == $userSignedIn && $val['approval_five'] == 0) {
		...
		?>
			...
			<h3 class="large nobold black-font alignct light-red-font top-pull-5">To read your inbox messages, click on the message subject</h3>
			...
			<form action="" method="post" onsubmit="objDisplay('processbar')" autocomplete="off">
				<div class="block-element steel-blue-theme top-pull-7 right-pull-30 bottom-pull-10 left-pull-30">
					<span class="ln-display-box float-left">
						<b class="fa-recycle nobold white-font"></b><input type="submit" name="archivebutton" value="Turn to archive" class="no-back-white anchor right-push-15">
					...
					<span class="ln-display-box float-right top-pull-3">
						<a href="?r=2" class="white-font ft-sml-size"><b class="fa-arrow-left nobold white-font"></b>&nbsp; Back</a>
					</span>
				...
				<div class="block-element box-border-thick-bottom top-pull-7 right-pull-30 bottom-pull-10 left-pull-30 ft-xsml-size">
					...
					<span class="ln-display-box float-right">
						<a href="?r=2" class="blue-font right-push-15">All</a><a href="?r=1" class="blue-font right-push-15">Read</a><a href="?r=3" class="blue-font">UnRead</a><a href="?r=5" class="default-text-font-bold royal-blue-font left-push-15">Unsigned</a> | <a href="?r=6" class="default-text-font-bold red-font">Archived</a>
					</span>
					<span class="block-element new-line-space">
					</span>
				</div>
				...
				<div class="block-element pads10 nc-height-85 y-scroll">
					...
					if(!isset($_GET['read_msg']))
					{
						...
						?>
							<table cellpadding="0" cellspacing="0">
								<tr>
									...
									<th width="200px" align="center">Received By</th>
								...
								<?php
									...
									if(isset($_GET['r']) && $_GET['r'] == 1) {
										...
										if(isset($_SESSION['archive'])) { unset($_SESSION['archive']); }
									} elseif(isset($_GET['r']) && $_GET['r'] == 2) {
										$_SESSION['keywords'] = " AND read_status IN(0,1,2) ORDER BY id DESC";
										if(isset($_SESSION['archive'])) { unset($_SESSION['archive']); }
									} elseif(isset($_GET['r']) && $_GET['r'] == 3) {
										$_SESSION['keywords'] = " AND read_status IN(0) ORDER BY id DESC";
										if(isset($_SESSION['archive'])) { unset($_SESSION['archive']); }
									} elseif(isset($_GET['r']) && $_GET['r'] == 4) {
										$_SESSION['keywords'] = " AND subject LIKE '%{$_GET['keyword']}%' ORDER BY id DESC";
										//if(isset($_SESSION['archive'])) { unset($_SESSION['archive']); }
									} elseif(isset($_GET['r']) && $_GET['r'] == 5) {
										$_SESSION['keywords'] = " AND read_status IN(2) ORDER BY id DESC";
										if(isset($_SESSION['archive'])) { unset($_SESSION['archive']); }
									} elseif(isset($_GET['r']) && $_GET['r'] == 6) {
										$_SESSION['keywords'] = " AND read_status IN(0,1,2) ORDER BY id DESC";
										$_SESSION['archive'] = 1;
									} else { 
									...
									//if(is_array($get_inbox_data)) {
										...
										//foreach($get_inbox_data as $inkey => $invalue) {
											...
											if(isset($allowSeeInbox) && $allowSeeInbox == 200) {
												$inbox_selection_key_2 = array("deletedata"=>0);
											} else {
												$inbox_selection_key_2 = array("receiver"=>$userSignedIn,"deletedata"=>0);
											}
											
											if(isset($_SESSION['archive']) && $_SESSION['archive'] == 1) {
												$inbox_selection_key_2['archivedata'] = 1;
											} else {
												$inbox_selection_key_2['archivedata'] = 0;
											}
											...
											$subject=""; $sentby=""; $receiver=""; $priority=""; $type=""; $msg_color_tag=""; $msg_status="";
											...
											if(is_array($get_inbox_data_)) {
												foreach ($get_inbox_data_ as $in2key => $in2value) {
													...
													$receiver=idget_data($tbL7,$in2value['receiver'],'staffname');
													...
													?>
														...
														<tr bgcolor="<?php echo $trcolor; ?>">
															...
															<td width="300px" align="left" class="box-border-dark-thick-right">
																<a href="?read_msg=<?php echo $in2value['id']; ?>" class="blue-font"><?php echo $subject; ?></a>
															</td>
						...
						$additionalQuery = " AND read_status IN(2)";
					...
					else
					{
						...
						?>
							<div class="block-element box-border-thick-bottom right-pull-10 bottom-pull-50 left-pull-10">
								<h2 class="large"><?php echo $subject; ?></h2>
								<?php echo $get_msg_data[4]; ?>
								<div class="cs-height-20"></div>
								<span class="float-left cs-height-40 top-pull-3 right-push-10"><b class="fa-clock nobold ft-large-size"></b></span>
								<h4 class="large nobold dark-grey-font bottom-pull-3">Delivery Date & Time</h4>
								<h4 class="large nobold"><?php echo date("d/m/y",strtotime($get_msg_data[7])).' '.$get_msg_data[8]; ?></h4>
							</div>
						<?php
					}
