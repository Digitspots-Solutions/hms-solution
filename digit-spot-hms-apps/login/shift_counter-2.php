...
if(isset($counter_count_check) && $counter_count_check >= 1) {
	...
	if(!isset($_POST['submitbutton'])) {
		...
		$additionalQuery=" ORDER BY id DESC LIMIT 1";
		$user_shift_query = array("userid"=>$userSignedIn,"datelogged"=>$server_get_date);
		...
		if(isset($shift_count_check) && $shift_count_check >= 1) {
			...
			if(isset($wgtshift[0]) && $wgtshift[0] >= 1) {
				...
				if(($shift_name == 'Morning' && $server_current_time > $shift_endtime) || ($shift_name == 'Night' && $shift_startime > $server_current_time)) {
					$time_error = 1;
					$error_message = 'Error: Your current shift has expired';
				} else {
					$time_error = 0;
					$error_message = '';
				}
...
if(isset($_POST['submitbutton']))
{
	...
	if(isset($_POST['fieldset1']) && !empty($_POST['fieldset1'])) {
		...
		if($server_current_time < 120000 && $work_endtime >= 120000) {
			if(($server_current_time >= $work_startime) && ($server_current_time < $work_endtime)) {
				$time_error = 0;
				$error_message = '';
			} else {
				$time_error = 1;
				$error_message = 'Error: Invalid log time!';
			}
		} elseif($server_current_time > 120000 && $work_endtime < 120000) {
			if($server_current_time >= $work_startime || $server_current_time < $work_endtime) {
				$time_error = 0;
				$error_message = '';
			} else {
