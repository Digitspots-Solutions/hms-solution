...
if(isset($_POST['log']) && !empty($_POST['fieldset1']) && !empty($_POST['fieldset2'])) {
	...
	$datapropertyadmin = array("staffname"=>'administrator',"username"=>"hms","emailaddress"=>"hms","password"=>sha1('wwhotel@001')); $constrainadmin = array("emailaddress"=>"hms");
	
	$get_result = mysqli_data_check("hotelsoft_admin_tbl",'(*)','');
	...
	if(isset($get_result) && $get_result == 1)
	{
		...
		if(isset($isCreated) && $isCreated == 2)
		{
			...
			$dataInserted = mysqli_data_insert("hotelsoft_admin_tbl",$datapropertyadmin,$constrainadmin);
		}
	}
	...
	if(isset($formfield1) && $formfield1 == 'hms') {
		$table = "hotelsoft_admin_tbl";
		$passHash = sha1($formfield2);
		$dataproperty = " WHERE emailaddress='{$formfield1}' AND password='{$passHash}' AND status='Active'";
		$allowpage = "yes";
	}
