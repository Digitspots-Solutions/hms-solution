<div class="block-element bottom-push-30">
 	<span class="ln-display-box float-left">
		Note: create your module category by selecting the module type
 	</span>
...
<?php
	...
	{
		createDatabasetable($var_tbl_4); //create a table for this post

		$fieldset1 = escape_data($_POST['fieldset1']);
		$fieldset2 = escape_data($_POST['fieldset2']);
		

		$insert_dataproperty = array("moduleid"=>$fieldset1,"category"=>ucwords(strtolower($fieldset2)));
		$insert_constrain = "";
		$data_inserted = mysqli_data_insert($tbL10,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"message"=>"Create new module category","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['editbutton']))
	{
		...
		$insert_dataproperty = array("moduleid"=>$fieldset1,"category"=>$fieldset2);
		$insert_constrain = array("id"=>$fieldset3);
		$data_inserted = mysqli_data_update($tbL10,$insert_dataproperty,$insert_constrain);
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"message"=>"Edit one of the module categories","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['deletebutton']) && isset($_POST['checkers']))
	{
		...
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"message"=>"Remove one, two or more of the module categories","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	if(isset($_POST['statusbutton']) && isset($_POST['checkers']))
	{
		...
		{
			$post_result .= '<span class="red-font">Unable to change record status. Try again</span>';
		}
		elseif(isset($data_updated) && $data_updated >= 1)
		{
			//create a log file
			$log_datasets = array("userid"=>$userSignedIn,"message"=>"Disable one, two or more of the module categories","datelogged"=>$server_get_date,"timelogged"=>$server_get_time); mysqli_data_insert($tbL8,$log_datasets,'');
	...
	$dataproperty = "id,moduleid,category,status";
	$constrain = "";
	$row = "array";

	$dataCollect = mysqli_data_fetch($tbL10,$dataproperty,$constrain,$row);

	if(is_array($dataCollect))
	{
		$thproperty = array("noth","module","category","status","noth","enoth");
		...
		foreach($dataCollect as $theader => $tdata)
		{
			...
			$dataid = $tdata["id"];
			$module_select=appmodule($tdata["moduleid"]);
			...
			if($tdata["status"] == 'Active') { $htmlresult .= '<td width="150px" align="center" class="box-border-thick-right leaf-green-font">'.$tdata["status"].'</td>'; } else { $htmlresult .= '<td width="150px" align="center" class="box-border-thick-right red-font">'.$tdata["status"].'</td>'; }
	...
	<div id="modal-box-1" class="nc-width-40 white-theme top-pull-30 right-pull-30 bottom-pull-30 left-pull-30 sml-rounded-button obj-light-shadow motion">
		<form action="" method="post" autocomplete="off">
			<div class="form-block-label alignlt">
				<h3 class="nobold nomargin">New Module Category</h3><br>
			</div>
			...
			<span class="block-element bottom-push-10">
				<input type="text" name="fieldset2" id="fieldset2" placeholder="Enter category" required="required">
			</span>
			<br>
...
<?php
	...
	if(isset($_GET['edit']) && $_GET['edit'] >= 1)
	{
		...
		$edatasets = "moduleid,category";
		$edataid = array("id"=>$fieldset1);
		$eget_data = mysqli_data_fetch($tbL10,$edatasets,$edataid,'noarray');

		$module_e_select=appmodule($eget_data[0]);
		...
		?>
			<div id="e-modal-box" class="fx-position-flow fscr zind-1 motion txp5-white" align="center">	
				...
				<div class="nc-width-40 white-theme top-pull-30 right-pull-30 bottom-pull-30 left-pull-30 sml-rounded-button obj-light-shadow motion">
					<form action="" method="post" autocomplete="off">
						<div class="form-block-label alignlt">
							<h3 class="nobold nomargin">Edit Module Category</h3><br>
						</div>
						<span class="block-element bottom-push-10">
							<select name="efieldset1" id="efieldset1" required="required">
								<option value="<?php echo $eget_data[0]; ?>"><?php echo $module_e_select; ?></option>
								<?php echo $apmOpt; ?>
							</select>
						</span>
						<span class="block-element bottom-push-10">
